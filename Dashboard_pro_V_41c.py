...
# ── v41c: Backup/Restore watchlist su file JSON locale ───────────────────────
...
def render_scan_tab(df,status_filter,sort_cols,ascending,title):
    ...
    if st.button(f"➕ Aggiungi a '{st.session_state.current_list_name}'",key=f"btn_{title}"):
        if not selected_df.empty and "Ticker" in selected_df.columns:
            ...
            _wl_save_backup()  # v41c: backup locale immediato
...
# TABS
# =========================================================================
# ── v41c: Menu sticky multi-riga — tutti i tab visibili su 3 righe ──────
st.markdown("""<style>
/* Sticky tab bar */
[data-testid="stTabs"] {
...
with tab_home:
    # ── v41c — ALERT BANNER: segnali STRONG visibili subito ──────────────
    try:
    ...
    # ── v41c — MERCATI LIVE GLOBALE con mini sparkline 5gg ─────────────────
    ...
    # ── v41c FEATURE 5 — MAPPA CALORE GLOBALE SVG ─────────────────────────
    with st.expander("🌍 Mappa Calore Globale — Performance indici mondiali", expanded=False):
        try:
    ...
    st.markdown('---')
    with st.expander('📰 NEWS & SENTIMENT v41c — Ultime news con score sentiment · TradingView Italia', expanded=False):
        _render_news_v41(df_ep)
    st.markdown('---')
    # ── v41c FEATURE 3 — HEATMAP SETTORIALE LIVE ──────────────────────────
    with st.expander("🌡️ Heatmap Settoriale Live — 11 settori × 3 periodi", expanded=True):
        @st.cache_data(ttl=300, show_spinner=False)
        def _sector_heatmap_home_v41c():
        ...
        _ht_period = st.radio("Periodo heatmap", ["1d","1m","3m"], index=0,
                               ...
                               label_visibility="collapsed")
        try:
            _ht_data = _sector_heatmap_home_v41c()
            if _ht_data:
    ...
    # ── v41c: Scanner Summary — segnali rapidi visibili dalla home ──────────
    if not df_ep.empty:
        st.markdown("---")
    ...
    # ── v41c FEATURE 4 — Portfolio P&L dalla Watchlist ──────────────────────
    try:
    ...
    st.markdown('<div class="section-pill">🗓️ MACRO CALENDAR v41c — Fed · CPI · NFP · PCE</div>', unsafe_allow_html=True)
