...
st.markdown('<div class="section-pill">SCANNER V45.09 · OPPORTUNITY RADAR · INSIDER RADAR · WATCHLIST ALERT · P&L TRACKER · BACKTEST PRO · EXPORT PRO · CHART TV-STYLE · MTF MATRIX · JOURNAL · REGIME</div>',unsafe_allow_html=True)
init_db()
...
if not only_watchlist:
    if st.button("🚀 AVVIA SCANNER PRO 45.09",type="primary",use_container_width=True):
...
def _render_pattern_alerts_v41(df_src, tab_name="x"):
    ...
    with _alert_col1:
        st.download_button("📊 Export Alert",_df_exp.to_csv(index=False).encode(),f"Alert_v45_09_{tab_name}_{_at_ts}.csv","text/csv",key=f"alert_exp_{tab_name}")
    with _alert_col2:
        st.download_button(
            "📺 TradingView TXT",
            data=make_tv_txt(_df_exp, section=f"ALERT_{tab_name}"),
            file_name=f"Alert_v45_09_{tab_name}_TradingView_{_at_ts}.txt",
            mime="text/plain",
...
with tab_home:
    ...
    with _v4504_export_col1:
        st.download_button(
            ...
            file_name="trading_scanner_v45_09_home.csv",
            mime="text/csv",
            key="v4504_home_csv",
            use_container_width=True,
        )
    with _v4504_export_col2:
        st.download_button(
            ...
            file_name="trading_scanner_v45_09_home.xlsx",
    ...
    with _v4504_export_col3:
        st.download_button(
            "📺 TradingView TXT",
            data=_v4504_home_tv,
            file_name="trading_scanner_v45_09_home_TradingView.txt",
            mime="text/plain",
            key="v4504_home_tv",
            use_container_width=True,
            disabled=(
...
# =========================================================================
st.markdown("---")
st.markdown('<div class="section-pill">💾 EXPORT PRO v45.09 — XLSX Multi-Sheet · TXT TradingView · Timestamp Auto</div>',unsafe_allow_html=True)
...
with ec1:
    st.download_button(
        ...
        f"TradingScanner_v45_09_Tutti_{_ts}.xlsx",
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        key="xlsx_all",
...
with ec2:
    ...
    st.download_button(
        "📥 TXT TV Tutti",
        data=_tv_all,
        file_name=f"TradingScanner_v45_09_Tutti_TradingView_{_ts}.txt",
        mime="text/plain",
        key="tv_txt_all",
...
with ec3:
    st.download_button(
        ...
        f"TradingScanner_v45_09_{cur_tab}_{_ts}.xlsx",
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        key="xlsx_curr"
    )
with ec4:
    if not df_cur.empty and "Ticker" in df_cur.columns:
        st.download_button(
            f"📥 TXT TV {cur_tab}",
            make_tv_txt(df_cur),
            f"TradingScanner_v45_09_{cur_tab}_TradingView_{_ts}.txt",
            "text/plain",
            key="tv_txt_curr"
        )
with ec5:
