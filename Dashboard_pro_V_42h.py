...
<button id='btt-btn' title='Torna all\'inizio'
  onclick='(function(){var s=window.parent.document.querySelector(\'section.main\')||window.parent.document.documentElement||document.body;s.scrollTo({top:0,behavior:\'smooth\'});})()'>&#8679;</button>
<script>
(function(){
  function _btt_attach(){
    ...
    if(!s){return;}
    function _btt_check(){
      var b=window.parent.document.getElementById('btt-btn')||document.getElementById('btt-btn');
      if(!b)return;
  ...
  [300,700,1500,3000,5000].forEach(function(d){setTimeout(_btt_attach,d);});
  try{var _btt_obs=new MutationObserver(_btt_attach);
    _btt_obs.observe(window.parent.document.body,{childList:true,subtree:true});}catch(e){}
})();
</script>"""
DARK_CSS = """
<style>
...
# PAGE CONFIG
# =========================================================================
st.set_page_config(page_title="Trading Scanner PRO 42.0h",layout="wide",page_icon="🧠")
st.markdown(DARK_CSS,unsafe_allow_html=True)
st.markdown(BACK_TO_TOP_CSS,unsafe_allow_html=True)
st.markdown("# 🧠 Trading Scanner PRO 42.0h")
...
with tab_home:
    ...
    # ── v42h: COT REPORT — Commitment of Traders ─────────────────────────
...
""", unsafe_allow_html=True)
    ...
    # ── v42h: Suggerimenti ────────────────────────────────
    with st.expander('💡 Suggerimenti v42h — Novità e roadmap', expanded=False):
        st.markdown("""
**✅ Implementato in v42h:**
...
with tab_rm:
    try:
        ...
        # ── v42h patch: pandas ≥2.1 applymap→map ─────────────────────────
        try:
...
with tab_ai2:
    ...
    st.markdown("<div class='section-pill'>🤖 MODULO 2 AI ANALYST v42h — Multi-source · Stile Momentum Alerts</div>", unsafe_allow_html=True)
    ...
    if _df_ai2.empty:
        ...
        _ai2_hcols = st.columns([1.4, 1.8, 0.9, 0.7, 0.7, 1.6])
        for _hc, _hl in zip(_ai2_hcols, ["Ticker", "Nome", "Stato", "CSS", "RSI", "Modulo 2"]):
            _hc.markdown(
        ...
        for _, _r in _df_ai2.head(60).iterrows():
            ...
            _c1,_c2,_c3,_c4,_c5,_c6 = st.columns([1.4,1.8,0.9,0.7,0.7,1.6])
            _c1.markdown(
            ...
            _c3.markdown(
                ...
                f"padding:2px 7px;font-size:0.72rem;font-weight:bold'>{_stato or '—'}</span>",
                unsafe_allow_html=True)
            _c4.markdown(
