...
# Renderer per MarketCap_fmt (stringa già formattata: "1.2B", "—")
...
with tab_w:
  ...
  <div style="margin-top:8px">{trend_b}{rsi_b}{vr_b}{qs_b}{ser_b}{fv_b}{sq_b}{wb_b}</div>
    ...
    if not _df_tv.empty:
        _tv_lines = _df_tv["ticker"].dropna().unique().tolist()
        st.download_button(
            label="📺 Export TradingView CSV",
            data=chr(10).join(_tv_lines),
            file_name="watchlist_tradingview.csv",
            mime="text/plain",
            key="wl_tv_export",
            help="Un ticker per riga — importabile in TradingView"
        )
