...
st.caption(
    ...
    "Multi‑Timeframe • Risk & Portfolio • Backtest PRO • Alert Designer • Cruscotto Giornaliero"
)
...
with tab_mtf:
    ...
    if df_ep.empty:
        ...
        if df_mtf.empty:
            ...
            # Selezione per Watchlist (es. solo ALIGN_LONG)
...
# =============================================================================
# TAB RISK & PORTFOLIO (come in PRO 5.0)
# =============================================================================
with tab_risk:
...
with tab_backtest:
    ...
    st.markdown(
        "Backtest semplice: entra quando un titolo ha Stato = PRO, "
        "esci quando il prezzo chiude sotto la EMA20 o l'RSI scende sotto una soglia."
    )

    if df_pro_all.empty:
        st.caption("Nessun segnale PRO disponibile per il backtest.")
    else:
        ...
        @st.cache_data(ttl=3600)
        def run_backtest_pro(tickers, years, rsi_exit):
            results = []
            ...
            for tkr in tickers:
                try:
                    ...
                    for i in range(21, len(data)):
                        ...
                        # entry: prezzo sopra EMA20 e RSI in range PRO (p_rmin,p_rmax)
                        if not in_pos and price > e20 and p_rmin < rsi_val < p_rmax:
                            in_pos = True
                            entry_price = price
                            continue

                        # exit: sotto EMA20 o RSI < rsi_exit
                    ...
                    if trades:
                        ...
                        results.append({
                            ...
                            "Best_Trade_%": round(trades_arr.max(), 2),
                            "Worst_Trade_%": round(max_dd, 2),
        ...
        if st.button("▶️ Esegui Backtest PRO"):
            with st.spinner("Backtest in corso..."):
                df_bt = run_backtest_pro(tickers_bt, years, rsi_exit)
            if df_bt.empty:
...
with tab_alert:
    ...
    sources = {
        "EARLY (top)": df_early_all.sort_values("Early_Score", ascending=False).head(top),
        "PRO (top)": df_pro_all.sort_values("Pro_Score", ascending=False).head(top),
        "REA_HOT (top)": df_rea_all.sort_values("Rea_Score", ascending=False).head(top),
    }
    if "df_mtf" in locals() and not df_mtf.empty:
        sources["MTF ALIGN_LONG"] = df_mtf[df_mtf["Segnale_MTF"] == "ALIGN_LONG"].sort_values("MTF_Score", ascending=False).head(top)
    ...
    alert_comment = st.text_input("Commento/Tag standard per alert (es: PRO6_MTF_LONG)", "PRO6_SIGNAL")
    ...
    if all_rows:
        ...
        csv_alert = df_alert[["symbol", "comment"]].to_csv(index=False, header=False).encode("utf-8")
        st.download_button(
        ...
        st.caption("Nessun simbolo selezionato per gli alert.")
...
with tab_daily:
    ...
    if not watch_rows:
        ...
        st.markdown("**Watchlist del giorno**")
        st.dataframe(df_daily, use_container_width=True)

        # Export: lista ticker + tipo
