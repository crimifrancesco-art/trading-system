# -*- coding: utf-8 -*-
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║         TRADING SCANNER PRO  —  v35.0                                  ║
# ║         Upgrade professionale su v34.0 — tutto il codice esistente      ║
# ║         è intatto. Aggiunti 6 upgrade v35 completamente additivi.       ║
# ╠══════════════════════════════════════════════════════════════════════════╣
# ║  CHANGELOG v35  (su base v34 intatta)                                  ║
# ║  #1  Scanner Engine v35 — worker pool auto-scaling su CPU reale,        ║
# ║      cache TTL differenziato (200s HOT / 600s EP), retry exp. backoff   ║
# ║      Score engine: soglie percentile dinamico (no più valori fissi)     ║
# ║  #2  Sidebar Pro v35 — filtri inline collassabili, Quick-Filter bar      ║
# ║      tasto RESET FILTRI, preset a icone, counter segnali live           ║
# ║  #3  Watchlist Pro v35 — alert engine (prezzo/score soglia) con         ║
# ║      badge visivo, P&L tracker con prezzo entrata + size + P&L%,        ║
# ║      mark-to-market aggiornato ad ogni refresh                          ║
# ║  #4  Backtest Pro v35 — Sharpe Ratio, Max Drawdown, Win Rate,           ║
# ║      Profit Factor, Avg R, equity curve con drawdown overlay            ║
# ║  #5  Export Pro v35 — xlsx multi-sheet con summary, CSV TradingView     ║
# ║      con un click, nomi file con timestamp automatico                   ║
# ║  #6  Chart TradingView-style v35 — VWAP intraday, livelli S/R auto,     ║
# ║      candele Heikin-Ashi opzionali, range selector 1W/1M/3M/6M/1Y      ║
# ╚══════════════════════════════════════════════════════════════════════════╝
import io
import time
import sqlite3
...
try:
    ...
    def scan_universe(universe, e_h, p_rmin, p_rmax, r_poc,
        ...
        try:
            ...
            _cpu_count = 4
        n = len(universe)
        # Formula: workers = min(requested, max(4, cpu*2), cap_per_size)
        _size_cap = 24 if n > 300 else 20 if n > 150 else 16 if n > 80 else 12
        ...
        def _one_cached(tkr):
            ...
            with lock:
                counter[0] += 1
                if progress_callback: progress_callback(counter[0], n, tkr)
            return ep, rea
        ...
        if not df_ep.empty and "Pro_Score" in df_ep.columns:
            ...
            if len(_scores) > 0:
                _p40 = float(_scores.quantile(0.40))  # bottom 40% escluso
                _p80 = float(_scores.quantile(0.80))  # top 20% = STRONG
                # Ricalibra Stato_Pro con soglie percentile se differiscono troppo dai fissi
                if _p40 < 4.0:   # batch di bassa qualità → abbassa threshold
