...
# ║  #3  Composite Signal Score   — colonne CSS, CSS_Grade                  ║
# ║      Score 0-100 che unifica Pro/Ser/FV + ADX + ATR + Liq + OBV        ║
# ║      Grade: A≥80 | B≥60 | C≥40 | D<40. Pinnato a destra in griglia.   ║
...
try:
    ...
    def scan_universe(universe, e_h, p_rmin, p_rmax, r_poc,
        ...
        # ── v33 SCANNER OTTIMIZZATO ──────────────────────────────────────
        # Miglioramenti rispetto alla v30:
        #  1. Cache per-ticker con TTL 15min (SQLite in-memory dict)
        #     → re-scan parziale risparmia ~50% chiamate Yahoo
        #  2. Batch sizing adattivo: universe grandi → più worker
        #  3. Timeout aggressivo (8s) + retry silenzioso
        #  4. Dedup risultati per ticker (evita duplicati in output)
        import concurrent.futures, threading, time, hashlib

        # ── Cache in-memory per-ticker (TTL 15 min = 900s) ──────────────
        _CACHE_TTL = 900
        if not hasattr(scan_universe, "_ticker_cache"):
            scan_universe._ticker_cache = {}
        _tc = scan_universe._ticker_cache

        def _cache_get(tkr):
            entry = _tc.get(tkr)
            if entry and (time.time() - entry["ts"]) < _CACHE_TTL:
                return entry["ep"], entry["rea"], True   # (ep, rea, hit)
            return None, None, False

        def _cache_set(tkr, ep, rea):
            _tc[tkr] = {"ep": ep, "rea": rea, "ts": time.time()}
        ...
        def _one(tkr):
            # 1. Prova cache
            ep, rea, hit = _cache_get(tkr)
            if hit:
                with lock:
                    counter[0] += 1
                    cache_hits[0] += 1
                    if progress_callback:
                        progress_callback(counter[0], len(universe), f"[cache] {tkr}")
                return ep, rea

            # 2. Fetch reale con retry
            for _attempt in range(2):
                try:
                    ep, rea = scan_ticker(tkr, e_h, p_rmin, p_rmax, r_poc, vol_ratio_hot)
                    _cache_set(tkr, ep, rea)
                    break
                except Exception:
                    if _attempt == 0:
                        time.sleep(0.3)
        ...
        # ── Worker count adattivo: più ticker → più worker ───────────────
        n = len(universe)
        nw = min(max(n_workers, 1), 16)
        if n > 200:  nw = min(nw, 16)
        elif n > 80: nw = min(nw, 12)
        else:        nw = min(nw, 8)

        seen_tickers = set()  # dedup output
        with concurrent.futures.ThreadPoolExecutor(max_workers=nw) as ex:
            fut_map = {ex.submit(_one, t): t for t in universe}
...
# Identica in tutto il progetto: main dashboard, home_tab, orderflow_tab
# ══════════════════════════════════════════════════════════════════════════════
BB_BG      = "#070b14"      # sfondo primario — nero Bloomberg profondo
BB_PANEL   = "#0d1117"      # pannelli interni
BB_PANEL2  = "#111923"      # pannelli secondari / hover
BB_BORDER  = "#1c2333"      # bordi sottili
BB_BORDER2 = "#243044"      # bordi accent
BB_TEXT    = "#c8d0e0"      # testo principale
BB_GRAY    = "#5a6478"      # testo secondario
BB_ORANGE  = "#ff6d00"      # accent primario Bloomberg (ticker bar, alert)
BB_TEAL    = "#00d4aa"      # positivo / green Bloomberg
BB_RED     = "#ff3d57"      # negativo
BB_AMBER   = "#ffb800"      # warning / gold
BB_BLUE    = "#2979ff"      # link / highlight
BB_CYAN    = "#00b8d4"      # header accent
BB_PURPLE  = "#7c4dff"      # divergenza / special
BB_MONO    = "'IBM Plex Mono','Courier New',monospace"
BB_SANS    = "'IBM Plex Sans','Trebuchet MS',sans-serif"

DARK_CSS = """
<style>
/* ══ BLOOMBERG TERMINAL SKIN v33 ═══════════════════════════════════════════ */
...
:root {
  ...
  --bb-sans:    'IBM Plex Sans','Trebuchet MS',sans-serif;
}
...
[data-testid="stSidebar"] *{color:var(--bb-text) !important;}
[data-testid="stSidebar"] .stSlider label,
[data-testid="stSidebar"] .stCheckbox label{
    font-family:var(--bb-mono) !important;font-size:0.75rem !important;
    color:var(--bb-gray) !important;}

/* ── HEADINGS ── */
h1{color:var(--bb-orange) !important;
   font-family:var(--bb-mono) !important;
   letter-spacing:2px;font-size:1.1rem !important;
   text-transform:uppercase;
   text-shadow:0 0 20px rgba(255,109,0,0.3);}
h2{color:var(--bb-cyan) !important;font-family:var(--bb-mono) !important;
   font-size:0.9rem !important;letter-spacing:1.5px;text-transform:uppercase;}
h3{color:var(--bb-amber) !important;font-family:var(--bb-mono) !important;
   font-size:0.82rem !important;letter-spacing:1px;}
.stCaption,small{color:var(--bb-gray) !important;font-family:var(--bb-mono) !important;}
...
[data-testid="stTabs"] button:hover{
    color:var(--bb-text) !important;
    background:var(--bb-panel2) !important;}
...
[data-testid="stMetricDelta"]{font-family:var(--bb-mono) !important;font-size:0.78rem !important;}
...
[data-testid="stButton"]>button[kind="secondary"]{
    ...
    border:1px solid rgba(255,61,87,0.35) !important;}
...
[data-testid="stExpander"] summary{
    color:var(--bb-cyan) !important;
    font-family:var(--bb-mono) !important;
    font-size:0.78rem !important;letter-spacing:0.5px;}
hr{border-color:var(--bb-border) !important;}
...
.ag-header-cell-label{
    ...
    letter-spacing:1px;
    text-transform:uppercase;}
.ag-header-cell-resize{background:var(--bb-border2) !important;}
.ag-row{
    background:var(--bb-panel) !important;
    border-bottom:1px solid var(--bb-border) !important;}
.ag-row:hover{background:var(--bb-panel2) !important;}
.ag-row-selected{
...
::-webkit-scrollbar-thumb{background:var(--bb-border2);border-radius:2px;}
::-webkit-scrollbar-thumb:hover{background:var(--bb-orange);}

/* ── BLOOMBERG SECTION PILL ── */
.section-pill{
...
.bb-panel{
    ...
    border-top:2px solid var(--bb-orange);
    border-radius:2px;
    padding:10px 14px;}
.bb-panel-cyan{
    background:var(--bb-panel);
    border:1px solid var(--bb-border);
    border-top:2px solid var(--bb-cyan);
    border-radius:2px;
    padding:10px 14px;}
.bb-panel-teal{
    background:var(--bb-panel);
    border:1px solid var(--bb-border);
    border-top:2px solid var(--bb-teal);
    border-radius:2px;
    padding:10px 14px;}
.bb-label{
    font-family:var(--bb-mono);
...
.bb-value{
    ...
    font-weight:700;
    color:var(--bb-text);}
.bb-pos{color:var(--bb-teal) !important;}
.bb-neg{color:var(--bb-red) !important;}
.bb-warn{color:var(--bb-amber) !important;}
.bb-accent{color:var(--bb-orange) !important;}

/* ── WATCHLIST CARDS Bloomberg ── */
.wl-card{
    background:var(--bb-panel);
    border:1px solid var(--bb-border);
    border-left:3px solid var(--bb-orange);
...
.wl-card:hover{border-color:var(--bb-cyan);}
.wl-card-ticker{
    font-family:var(--bb-mono);
    font-size:0.95rem;
    font-weight:700;
    color:var(--bb-orange);
    letter-spacing:2px;}
.wl-card-name{color:var(--bb-gray);font-size:0.78rem;margin-top:2px;font-family:var(--bb-mono);}
.wl-card-badge{display:inline-block;border-radius:2px;padding:1px 6px;font-size:0.68rem;font-weight:700;margin-right:4px;font-family:var(--bb-mono);}
.badge-green{background:rgba(0,212,170,0.12);color:var(--bb-teal);border:1px solid rgba(0,212,170,0.3);}
.badge-orange{background:rgba(255,109,0,0.12);color:var(--bb-orange);border:1px solid rgba(255,109,0,0.3);}
.badge-red{background:rgba(255,61,87,0.12);color:var(--bb-red);border:1px solid rgba(255,61,87,0.3);}
.badge-blue{background:rgba(41,121,255,0.12);color:var(--bb-blue);border:1px solid rgba(41,121,255,0.3);}
.badge-gray{background:rgba(90,100,120,0.12);color:var(--bb-gray);border:1px solid rgba(90,100,120,0.3);}
.badge-purple{background:rgba(124,77,255,0.12);color:var(--bb-purple);border:1px solid rgba(124,77,255,0.3);}

/* ── LEGEND TABLE Bloomberg ── */
.legend-table{width:100%;border-collapse:collapse;font-family:var(--bb-mono);font-size:0.76rem;}
.legend-table th{
    color:var(--bb-orange);
    border-bottom:1px solid var(--bb-border);
    padding:5px 10px;text-align:left;
    letter-spacing:1px;text-transform:uppercase;font-size:0.66rem;}
.legend-table td{color:var(--bb-text);border-bottom:1px solid var(--bb-border);padding:4px 10px;}
.legend-table tr:hover td{background:var(--bb-panel2);}
.legend-col-name{color:var(--bb-teal);font-weight:700;}
.legend-col-range{color:var(--bb-amber);}
.crit-ok{color:var(--bb-teal);font-weight:700;}
.crit-no{color:var(--bb-red);}
</style>
"""

# Bloomberg Terminal Plotly theme — usato ovunque
PLOTLY_DARK = dict(
    paper_bgcolor=BB_BG,
    plot_bgcolor=BB_PANEL,
    font=dict(color=BB_TEXT, family="IBM Plex Mono, Courier New, monospace", size=11),
    xaxis=dict(gridcolor=BB_BORDER, zerolinecolor=BB_BORDER2,
               linecolor=BB_BORDER, tickfont=dict(color=BB_GRAY, size=9)),
    yaxis=dict(gridcolor=BB_BORDER, zerolinecolor=BB_BORDER2,
               linecolor=BB_BORDER, tickfont=dict(color=BB_GRAY, size=9)),
)
# =========================================================================
...
if not only_watchlist:
    if st.button("🚀 AVVIA SCANNER PRO 31.1",type="primary",use_container_width=True):
        ...
        if not universe:
            ...
            ch      = scan_stats.get("cache_hits", 0)
            dl      = scan_stats.get("downloaded", tot)
            nw_used = scan_stats.get("workers", n_wk)

            _cache_note = f" | ⚡ **{ch}** cache hits" if ch > 0 else ""
            status.success(
                f"✅ **{tot} ticker** in **{elapsed:.0f}s**{_cache_note} — "
                f"🌐 **{dl}** fetch reali · **{nw_used}** worker — "
