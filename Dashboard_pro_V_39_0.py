...
# ║         è intatto. Aggiunti 9 upgrade v39 completamente additivi.       ║
# ╠══════════════════════════════════════════════════════════════════════════╣
# ║  CHANGELOG v39  (su base v37 intatta)                                  ║
...
# =========================================================================
# v37/v39 — DEFINIZIONI FUNZIONI (spostate prima dei tab per evitare NameError)
# =========================================================================
...
def _detect_patterns_v39(row: pd.Series) -> list:
    """
...
def _render_pattern_alerts_v39(df_ep_alerts, tab_name="default"):
    """Tab Alert Multipli: mostra pattern attivi + configurazione soglie."""
    st.markdown('<div class="section-pill">🔔 ALERT MULTIPLI v39 — Pattern Tecnici Real-Time</div>',
                unsafe_allow_html=True)
    ...
    st.download_button("📊 Export Alert",
        ...
        f"PatternAlerts_v39_{_at_ts}.csv", "text/csv",
        key=f"alert_export_v39_{tab_name}")


# =========================================================================
# v39 UPGRADE #4 — NEWS & SENTIMENT ENGINE
# Feed RSS Yahoo Finance + score NLP sentiment Bull/Bear/Neutral
# =========================================================================

@st.cache_data(ttl=600)
def _fetch_news_sentiment_v39(tickers: tuple) -> list:
    """
...
def _render_news_sentiment_v39(df_ep_news):
    """Tab News & Sentiment."""
    st.markdown('<div class="section-pill">📰 NEWS & SENTIMENT v39 — Feed Real-Time + NLP Score</div>',
                unsafe_allow_html=True)
    ...
    with st.spinner("Carico news..."):
        _news = _fetch_news_sentiment_v39(tuple(_ns_tickers))
...
# =========================================================================
# v39 UPGRADE #5 — SEC FORM 4 INSIDER BUYING
# =========================================================================

@st.cache_data(ttl=3600)
@st.cache_data(ttl=3600)
def _fetch_insider_buying_v39(tickers: tuple) -> list:
    """
...
@st.cache_data(ttl=3600)
def _fetch_short_interest_v39(tickers: tuple) -> dict:
...
# =========================================================================
# v39 UPGRADE #7 — MACRO CALENDAR
# =========================================================================

@st.cache_data(ttl=3600)
def _fetch_macro_calendar_v39() -> list:
    """
...
# =========================================================================
# v39 UPGRADE #8 — OPTIONS FLOW PROXY (put/call ratio)
# =========================================================================

@st.cache_data(ttl=900)
def _fetch_options_flow_v39(ticker: str) -> dict:
    """
...
# I nuovi tab v39 vengono aggiunti tramite st.tabs ridefinito con append
...
with tab_mtfmatrix:
    ...
    with _cc2:
        if not df_ep.empty and "Ticker" in df_ep.columns:
            ...
            # v39: mappa ticker → nome per il multiselect
            _cmp_nome_map = {}
            ...
            _scan_tkrs_sorted = sorted(_scan_tkrs,
                key=lambda t: _cmp_nome_map.get(t, t).lower())
            _cmp_extra = st.multiselect(
                ...
                help="Aggiungi ticker dalla tua ultima scansione — ordinati alfabeticamente per nome"
            )
        else:
            _cmp_extra = []
        st.write("")
...
# =========================================================================
    ...
    # ── v37/v39 additions for tab_rm ──
    st.markdown("---")
    _render_telegram_settings_v37()

# Risk Dashboard nel Risk Manager
    st.markdown("---")
    _render_risk_dashboard_v37(df_ep)

# Scanner Avanzato nel tab Order Flow (sezione dedicata)
    st.markdown("---")
    st.markdown('<div class="section-pill">📊 INTEGRAZIONI DATI v39</div>', unsafe_allow_html=True)
        ...
        if _of_tickers:
            ...
            if st.button("📉 Calcola P/C Ratio", key="of_calc"):
                with st.spinner(f"Options chain {_of_sel}..."):
                    _of_res = _fetch_options_flow_v39(_of_sel)
                if _of_res:
        ...
        if _si_tickers:
            if st.button("🩳 Carica Short Interest", key="si_load"):
                with st.spinner("Scarico short interest..."):
                    _si_data = _fetch_short_interest_v39(_si_tickers)
                if _si_data:
                    _df_si = pd.DataFrame([
        ...
        if _ins_tickers:
            if st.button("🏛️ Cerca Insider Buying", key="ins_load"):
                with st.spinner("Scarico dati SEC Form 4..."):
                    _ins_data = _fetch_insider_buying_v39(_ins_tickers)
                if _ins_data:
...
with tab_w:
    ...
    with st.expander("📈 P&L Tracker & Alert Engine v36", expanded=False):
        ...
        with _pnl_col1:
            ...
            if "v39_pnl_entries" not in st.session_state:
                st.session_state["v39_pnl_entries"] = {}
            _pnl = st.session_state["v39_pnl_entries"]
        ...
        with _pnl_col2:
            ...
            if "v39_alerts" not in st.session_state:
                st.session_state["v39_alerts"] = {}
            _alerts = st.session_state["v39_alerts"]
...
with tab_of:
    ...
    # ── v37/v39 additions for tab_of ──
    st.markdown("---")
    _render_advanced_scanner_v37()

# AI Explainer anche nel tab CONFLUENCE
with tab_bcd:
    try:
...
with ec1:
    st.download_button(
        ...
        f"TradingScanner_v39_Tutti_{_ts}.xlsx",
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        key="xlsx_all",
...
with ec2:
    ...
    if tv_rows:
        ...
        st.download_button(
            ...
            f"TradingScanner_v39_TV_{_ts}.csv",
            "text/csv",
            key="csv_tv_all",
...
with ec3:
    st.download_button(
        ...
        f"TradingScanner_v39_{cur_tab}_{_ts}.xlsx",
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        key="xlsx_curr"
    )
with ec4:
    if not df_cur.empty and "Ticker" in df_cur.columns:
        st.download_button(
            ...
            f"TradingScanner_v39_{cur_tab}_TV_{_ts}.csv",
            "text/csv",
            key="csv_tv_curr"
        )
with ec5:
    # v35: export P&L tracker se presente
    _pnl_data = st.session_state.get("v39_pnl_entries", {})
    if _pnl_data:
    ...
    if not _mtf_tickers:
        ...
        if _mtf_res:
            ...
            st.download_button("📊 Export MTF Matrix",
                ...
                f"MTF_Matrix_v39_{_mtf_ts}.csv", "text/csv", key="mtf_export")
...
with tab_journal:
    ...
    if "v39_journal" not in st.session_state:
        st.session_state["v39_journal"] = []
    _journal = st.session_state["v39_journal"]
    ...
    if _journal:
        ...
        with _jd2:
            if st.button("🗑️ Svuota tutto", key="j_clear_all"):
                st.session_state["v39_journal"] = []; st.rerun()
        ...
        st.download_button("📊 Export Journal XLSX",
            ...
            f"TradingJournal_v39_{_j_ts}.xlsx",
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="j_export_xlsx")
    else:
        ...
        if _run_gap:
            ...
            if _df_gaps.empty:
                ...
                st.download_button("📊 Export Gap Scanner",
                    ...
                    f"GapScanner_v39_{_gap_ts}.csv", "text/csv", key="gap_export")
...
# =========================================================================
# v39 TAB NUOVO — 💡 ANALISI PERSONALE
...
with tab_home:
    ...
    # ── v37/v39 additions for tab_home ──
    st.markdown("---")
    st.markdown('<div class="section-pill">📰 NEWS & SENTIMENT v39</div>', unsafe_allow_html=True)
    with st.expander("📰 Ultime news con score sentiment sui ticker scanner/watchlist",
                     expanded=False):
        _render_news_sentiment_v39(df_ep)

    st.markdown("---")
    st.markdown('<div class="section-pill">🗓️ MACRO CALENDAR v39 — Fed · CPI · NFP · PCE</div>',
                unsafe_allow_html=True)
    _macro_events = _fetch_macro_calendar_v39()
...
with tab_e:
    ...
    # ── v37/v39 additions for tab_e ──
    st.markdown("---")
    with st.expander("🔔 Alert Multipli v39", expanded=False):
...
# Telegram nel Risk Manager
    st.markdown("---")
    with st.expander("🔔 Alert Multipli v39 — Pattern tecnici rilevati", expanded=False):
...
with tab_conf:
    ...
    # ── v37/v39 additions for tab_conf ──
...
# =========================================================================
# v39 UPGRADE #2 — ALERT MULTIPLI ENGINE
# Pattern tecnici: EMA breakout, golden/death cross, squeeze fire,
# Bollinger breakout, RSI divergence, volume spike
# =========================================================================

with tab_analisi:
    st.markdown('<div class="section-pill">💡 ANALISI PERSONALE v39 — Carica i tuoi ticker · Ricevi consigli operativi AI</div>',
                unsafe_allow_html=True)
