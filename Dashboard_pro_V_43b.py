...
# PAGE CONFIG
# =========================================================================
st.set_page_config(page_title="Trading Scanner PRO 43.0b",layout="wide",page_icon="🧠")
st.markdown(DARK_CSS,unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════════════════════
# v43b — WEB PUSH NOTIFICATIONS helper (JS browser)
# ══════════════════════════════════════════════════════════════════════════════
_PUSH_JS = """
<script>
(function(){
    const PUSH_KEY = 'v43b_push_enabled';
    window._v43bPushEnabled = sessionStorage.getItem(PUSH_KEY)==='1';
    window._v43bRequestPush = function(){
        if(!('Notification' in window)){return;}
        Notification.requestPermission().then(p=>{
            if(p==='granted'){
                sessionStorage.setItem(PUSH_KEY,'1');
                window._v43bPushEnabled=true;
    ...
    window._v43bSendPush = function(title,body){
        if(window._v43bPushEnabled && Notification.permission==='granted'){
            new Notification(title,{body:body});
        }
    };
    document.addEventListener('DOMContentLoaded',function(){
        var btn=document.getElementById('v43b-push-btn');
        if(btn){
            btn.onclick=window._v43bRequestPush;
            if(window._v43bPushEnabled){btn.innerText='🔔 Push ON';}
        }
    });
})();
</script>
...
st.markdown("# 🧠 Trading Scanner PRO 43.0b")
...
with tab_home:
    ...
    try:
        ...
        # ══════════════════════════════════════════════════════════════════════════
    # v43b — COT REPORT: fetch automatico CFTC + grafici posizione netta
    # ══════════════════════════════════════════════════════════════════════════
    try:
        with st.expander("📊 COT Report — Commitment of Traders (CFTC, aggiornato venerdì)", expanded=False):
            ...
            def _fetch_cot_v43b(code, asset_name):
            ...
            for _ca in _cot_sel:
                ...
                _cdf = _fetch_cot_v43b(_ccode, _ca)
...
Dati CFTC pubblicati ogni <b>venerdì 21:30 CET</b>, riferiti al martedì precedente.
        ...
        with st.expander("📈 Pattern Recognition — Candlestick + Volume Profile", expanded=False):
            ...
            if _pr_go and _pr_tk:
                ...
                if _pr_df.empty:
                    ...
                    st.plotly_chart(fig_pr, use_container_width=True, key="v43b_pr_chart")
                    ...
                    if patterns_found:
                        ...
                        st.download_button("📥 CSV Pattern",
                            _pdf_patt.to_csv(index=False).encode("utf-8"),
                            f"pattern_{_pr_tk}_{datetime.now().strftime('%Y%m%d')}.csv",
                            "text/csv", key="v43b_pr_csv")
                    else:
    ...
    # ── v43b: COT REPORT — Commitment of Traders ─────────────────────────
...
""", unsafe_allow_html=True)
    ...
    # ── v43b: Suggerimenti ────────────────────────────────
    with st.expander('💡 Suggerimenti v43b — Novità e roadmap — Novità e roadmap', expanded=False):
        st.markdown("""
**✅ Implementato in v43b:**
...
with tab_rm:
    try:
        ...
        # ── v43b patch: pandas ≥2.1 applymap→map ─────────────────────────
        try:
...
with tab_ai:
    ...
    try:
        ...
        # ══════════════════════════════════════════════════════════════════════════
    # v43b — AI BATCH MULTI-TICKER SCHEDULATA
    # ══════════════════════════════════════════════════════════════════════════
    try:
        with st.expander("🤖 AI Batch — Analisi multi-ticker schedulata", expanded=False):
            ...
            with _batchc1:
                _batch_tks_raw = st.text_input(
                    ...
                    key="v43b_batch_tks", placeholder="es: UCG.MI,ISP.MI,RACE.MI")
            with _batchc2:
                _batch_mode = st.selectbox(
                    "Modello", ["Gemini (free)","Groq (free)","OpenRouter"],
                    key="v43b_batch_model")
            with _batchc3:
                _batch_go = st.button("🚀 Avvia Batch", key="v43b_batch_go",
                                      use_container_width=True)
            if _batch_go:
                ...
                if not _btks:
                    ...
                    # Export CSV batch
                    if _bresults:
                        st.download_button("📥 CSV Batch",
                            pd.DataFrame(_bresults).to_csv(index=False).encode("utf-8"),
                            f"ai_batch_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
                            "text/csv", key="v43b_batch_csv")
...
with tab_ai2:
    ...
    st.markdown("<div class='section-pill'>🤖 MODULO 2 AI ANALYST v43b — Multi-source · Vol× · Priorità · Stile Momentum Alerts</div>", unsafe_allow_html=True)
