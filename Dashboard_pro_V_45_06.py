...
# =========================================================================
# ── V45.05: tab Opportunity Radar ───────────────────────────────────────
with tab_radar:
    st.session_state["last_active_tab"] = "OPPORTUNITY_RADAR"
    st.title("🎯 Opportunity Radar")

    st.caption(
        "RSI e VWAP sono usati come segnali di screening, non come garanzia di acquisto: "
    ...
    # Import del modulo
    from modules.opportunity_radar import screen_opportunities, prepare_radar_data_for_demo

    # Per ora usiamo dati demo
    prices, market_caps, dollar_volumes, earnings_days, macro_ok, cfg = prepare_radar_data_for_demo()

    df_res = screen_opportunities(
        prices=prices,
    ...
    df_view = df_res[df_res["level"].isin(level_filter)].reset_index(drop=True)
