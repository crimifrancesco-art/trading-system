...
BACK_TO_TOP_CSS = ""  # replaced by component below
...
def _tv_symbol(raw_ticker):
    """Normalizza un ticker nel formato EXCHANGE:SYMBOL."""
    ...
    for suffix, exchange in mappings:
        if ticker.endswith(suffix):
            return f"{exchange}{ticker[:-len(suffix)]}"

    # Se l'origine non specifica l'exchange, non inventarlo.
    return ticker


def _tv_tickers_from_df(df, ticker_col="Ticker"):
    """Restituisce simboli TradingView deduplicati mantenendo l'ordine."""
    ...
    for raw_ticker in df[ticker_col].dropna().tolist():
        symbol = _tv_symbol(raw_ticker)
...
def make_tv_txt(df, section="PORTAFOGLIO"):
    """
    Formato TradingView:
    ###NOME_SEZIONE,EXCHANGE:TICKER,EXCHANGE:TICKER,
    """
    symbols = _tv_tickers_from_df(df)
    section = re.sub(r"[^A-Z0-9_-]+", "_", str(section).upper()).strip("_")
    section = section or "PORTAFOGLIO"

    if not symbols:
        return f"###{section},\n".encode("utf-8")

    return (f"###{section}," + ",".join(symbols) + ",\n").encode("utf-8")


def make_tv_csv(df, tab=""):
    # Mantiene la compatibilità con i punti esistenti del programma.
    return make_tv_txt(df, section=tab or "PORTAFOGLIO")


def make_tv_sections(groups):
    """
    groups: dict[str, DataFrame]
    Restituisce una riga TradingView per ogni gruppo.
    """
    lines = []

    for section, df in groups.items():
        symbols = _tv_tickers_from_df(df)
        safe_section = re.sub(
            r"[^A-Z0-9_-]+", "_", str(section).upper()
        ).strip("_") or "PORTAFOGLIO"

        if symbols:
            lines.append(f"###{safe_section}," + ",".join(symbols) + ",")
...
def csv_btn(df, fname, key):
    """Export CSV e TXT compatibile TradingView per il DataFrame visibile."""
    col_csv, col_tv = st.columns(2)
    ...
    with col_tv:
        ...
        st.download_button(
            "📺 TradingView TXT",
            data=make_tv_txt(df, section=section),
            file_name=f"{Path(fname).stem}_TradingView.txt",
            mime="text/plain",
            key=f"{key}_tv_txt",
            disabled=df is None or df.empty or "Ticker" not in df.columns,
            help="Formato TradingView: ###SEZIONE,ticker separati da virgole,",
        )
...
except Exception as _cot_import_error:
    _HAS_COT_EVOLVED = False
...
try:
    from utils.insider_radar import (
        fetch_recent_insider_transactions,
        fetch_insider_transactions_extended,
        compute_insider_score,
        role_bucket as _ir_role_bucket_fn,
    )
...
st.markdown('<div class="section-pill">SCANNER V45.11 · OPPORTUNITY RADAR · INSIDER RADAR PRO · WATCHLIST ALERT · P&L TRACKER · BACKTEST PRO · EXPORT PRO · CHART TV-STYLE · MTF MATRIX · JOURNAL · REGIME</div>',unsafe_allow_html=True)
init_db()
...
if not only_watchlist:
    if st.button("🚀 AVVIA SCANNER PRO 45.11",type="primary",use_container_width=True):
...
def _render_pattern_alerts_v41(df_src, tab_name="x"):
    ...
    _en = {pid: True for pid in _PATTERN_ALERTS_V39}
    if st.checkbox(
        "⚙️ Mostra pattern da monitorare",
        value=False,
        key=f"pattern_alerts_expand_{tab_name}",
    ):
        _pc = st.columns(4)
        _en = {
            pid: _pc[i % 4].checkbox(
                f"{p['icon']} {p['label']}",
                True,
                key=f"pat_en_{pid}_{tab_name}",
                help=p["desc"],
            )
    ...
    _alert_col1, _alert_col2 = st.columns(2)
    with _alert_col1:
        st.download_button("📊 Export Alert",_df_exp.to_csv(index=False).encode(),f"Alert_v45_11_{tab_name}_{_at_ts}.csv","text/csv",key=f"alert_exp_{tab_name}")
    with _alert_col2:
        st.download_button(
            "📺 TradingView TXT",
            data=make_tv_txt(_df_exp, section=f"ALERT_{tab_name}"),
            file_name=f"Alert_v45_11_{tab_name}_TradingView_{_at_ts}.txt",
            mime="text/plain",
            key=f"alert_exp_tv_{tab_name}",
            disabled=_df_exp.empty or "Ticker" not in _df_exp.columns,
            help="Formato TradingView: ###ALERT_<TAB>,ticker separati da virgole,",
        )
...
def _ai_call_with_fallback(prompt: str) -> tuple:
    ...
    try:
        ...
        _gem_key  = _ss.get("_gemini_api_key","")
        _groq_key = _ss.get("_groq_api_key","")
        _or_key   = _ss.get("_openrouter_api_key","")
        _ant_key  = _ss.get("_anthropic_api_key","")
    if _gem_key:
...
# ── V45.05: tab Opportunity Radar + Macro Regime ────────────────────────
tabs = st.tabs([
...
(tab_home, tab_e, tab_p, tab_r, tab_conf,
 ...
 tab_regime, tab_mtfmatrix, tab_mtf, tab_bcd, tab_of, tab_rm,
 tab_portfolio, tab_bt,
...
with tab_home:
    ...
    _v4504_home_tv = make_tv_txt(_v4504_home_export, section="HOME")

    _v4504_export_col1, _v4504_export_col2, _v4504_export_col3 = st.columns(3)
    with _v4504_export_col1:
        st.download_button(
            "⬇️ Export Home CSV",
            data=_v4504_home_csv,
            file_name="trading_scanner_v45_11_home.csv",
            mime="text/csv",
            key="v4504_home_csv",
            use_container_width=True,
        )
    with _v4504_export_col2:
        st.download_button(
            ...
            file_name="trading_scanner_v45_11_home.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="v4504_home_excel",
            use_container_width=True,
        )
    with _v4504_export_col3:
        st.download_button(
            "📺 TradingView TXT",
            data=_v4504_home_tv,
            file_name="trading_scanner_v45_11_home_TradingView.txt",
            mime="text/plain",
            key="v4504_home_tv",
            use_container_width=True,
            disabled=(
                _v4504_home_export.empty
                or "Ticker" not in _v4504_home_export.columns
            ),
            help="Formato TradingView: ###HOME,ticker separati da virgole,",
        )
    ...
    with st.expander("📊 COT Report Evoluto V45.032 — Posizionamento istituzionale", expanded=False):
        if _HAS_COT_EVOLVED:
            ...
            st.warning("Modulo COT Evoluto non disponibile. Verifica utils/cot_report.py e fai redeploy.")
            st.caption("Errore import: " + str(_cot_import_error))
...
- Eseguire `python -m py_compile Dashboard_pro_V_45_10.py` dopo ogni modifica.
...
- Usare COT e macro come filtri di contesto, insieme a prezzo, liquidità, volatilità, stop e position sizing.
    ...
    st.markdown("---")
    st.caption("Trading Scanner PRO · Versione V45.11 · Insider Radar Pro (scoring stabile, ruoli corretti, cluster temporali), Opportunity Radar, Macro Regime")
        ...
        if _df_insider_raw is None or _df_insider_raw.empty:
            ...
            with _ir_c2f:
                _ir_role_filter = st.multiselect(
                    "Ruolo",
                    options=["CEO/CFO", "Director/Officer", "10% Owner", "Altro"],
                    default=["CEO/CFO", "Director/Officer", "10% Owner", "Altro"],
            ...
            if not _df_view_insider.empty:
                ...
                _df_view_insider["RuoloGruppo"] = _df_view_insider["Ruolo"].apply(_ir_role_bucket_fn)
...
with tab_macro:
    ...
    if _HAS_MACRO_REGIME:
        ...
        st.error("Macro Regime Engine non disponibile.")
        st.caption("Errore import: " + str(_macro_import_error))
...
getGui(){return this.eGui;}refresh(){return false;}}"""))
    ...
    _crisis_ts = datetime.now().strftime("%Y%m%d_%H%M")
    _crisis_scenario_slug = (
        str(selected_scenario).strip().upper()
        .replace(" ", "_").replace("/", "_")[:32]
    )
    _cx1, _cx2, _cx3 = st.columns(3)
    with _cx1:
        st.download_button("📺 TradingView TXT",
            data=make_tv_txt(pd.DataFrame({"Ticker": _unique}), section=f"CRISIS_{_crisis_scenario_slug}"),
            file_name=f"Crisis_{_crisis_scenario_slug}_{_crisis_ts}.txt",
            mime="text/plain",
            key=f"crisis_tv_txt_{_crisis_scenario_slug}",
            disabled=not _unique,
            help="Formato TradingView: ###CRISIS_<SCENARIO>,ticker separati da virgole,")
    with _cx2:
...
with tab_portfolio:
    st.markdown(
        "<div class='section-pill'>💼 PORTAFOGLIO PERSONALE v45.11</div>",
        unsafe_allow_html=True,
    )

    st.caption(
        "Carica un export CSV/XLS/XLSX Fineco (di sintesi o dettagliato). "
        "L'analisi e' informativa e non genera ordini automatici. "
        "Il sistema riconosce i titoli dalle colonne del file: se il "
        "portafoglio cambia composizione nel tempo, basta caricare il "
        "nuovo export."
    )
    ...
    from utils.portfolio_engine import (
        ...
        portfolio_to_csv,
        portfolio_to_tradingview,
        portfolio_to_xlsx,
    )

    _portfolio_file = st.file_uploader(
        "Carica esportazione portafoglio",
        type=["csv", "xls", "xlsx"],
        key="portfolio_upload_v4511",
    )

    if _portfolio_file is None:
        st.info("Carica un file CSV, XLS o XLSX per iniziare.")
    else:
        try:
            _portfolio_raw = _read_upload(_portfolio_file)
            _portfolio_df = normalize_portfolio(_portfolio_raw)

            if _portfolio_df.empty:
                st.warning(
                    "Nessuna posizione riconosciuta automaticamente. "
                    "Se hai caricato il 'Portafoglio di sintesi', prova a "
                    "esportare il portafoglio dettagliato (con ISIN, "
                    "Quantita' e Valore di mercato) per un'analisi completa."
                )
            else:
                _has_values = has_market_values(_portfolio_df)

                if not _has_values:
                    st.warning(
                        "⚠️ Questo export contiene solo l'elenco degli "
                        "strumenti (senza Quantita'/Valore di mercato). "
                        "Pesi, valore totale e riequilibrio quantitativo "
                        "non sono disponibili: carica un export Fineco "
                        "dettagliato per calcolarli."
                    )

                _portfolio_total = float(_portfolio_df["Valore_mercato"].sum())

                _p1, _p2, _p3, _p4 = st.columns(4)
                _p1.metric("Posizioni", len(_portfolio_df))
                _p2.metric(
                    "Valore totale",
                    f"{_portfolio_total:,.2f} €" if _has_values else "n/d",
                )

                _equity_mask = _portfolio_df["Asset_Class"].isin(["Azionario", "ETF"])
                _crypto_mask = _portfolio_df["Asset_Class"].eq("Crypto")

                _p3.metric(
                    "Azionario/ETF",
                    f"{_portfolio_df.loc[_equity_mask, 'Peso_%'].sum():.1f}%" if _has_values else "n/d",
                )
                _p4.metric(
                    "Crypto",
                    f"{_portfolio_df.loc[_crypto_mask, 'Peso_%'].sum():.1f}%" if _has_values else "n/d",
                )

                _current_ids = set(_portfolio_df["Position_ID"].dropna().tolist())
                _previous_ids = set(st.session_state.get("_portfolio_prev_ids", []))

                if _previous_ids:
                    _diff = diff_portfolio_snapshots(_previous_ids, _current_ids)
                    if _diff["nuovi"] or _diff["usciti"]:
                        with st.expander("🔄 Variazioni rispetto al caricamento precedente", expanded=False):
                            if _diff["nuovi"]:
                                st.markdown(f"🆕 **Nuove posizioni:** {len(_diff['nuovi'])}")
                                st.write(sorted(_diff["nuovi"]))
                            if _diff["usciti"]:
                                st.markdown(f"📤 **Posizioni non piu' presenti:** {len(_diff['usciti'])}")
                                st.write(sorted(_diff["usciti"]))

                st.session_state["_portfolio_prev_ids"] = list(_current_ids)

                st.markdown("### Posizioni riconosciute")
                st.info(
                    "💱 Il prezzo di mercato è espresso nella valuta originale "
                    "del titolo (USD, EUR, GBP...). Il controvalore e il peso "
                    "sono calcolati in EUR usando il campo Fineco 'Valore di mercato €'."
                )

                _display_cols = [
                    "Nome", "ISIN", "Ticker", "Mercato", "Valuta",
                    "Asset_Class", "Settore", "Quantità", "Prezzo_mercato",
                    "Valore_mercato", "Peso_%", "Var_pct",
                ]
                _display_cols = [
                    c for c in _display_cols if c in _portfolio_df.columns
                ]
                _display = _portfolio_df[_display_cols].copy()
                _display = _display.rename(
                    columns={
                        "Valuta": "Valuta prezzo",
                        "Prezzo_mercato": "Prezzo mercato [valuta titolo]",
                        "Valore_mercato": "Controvalore [EUR]",
                        "Peso_%": "Peso portafoglio [%]",
                        "Var_pct": "Varianza [%]",
                    }
                )
                st.dataframe(
                ...
                _currency_summary = (
                    _portfolio_df.groupby("Valuta", dropna=False)
                    .agg(
                        Posizioni=("Position_ID", "count"),
                        Controvalore_EUR=("Valore_mercato", "sum"),
                    )
                    .reset_index()
                    .rename(
                        columns={
                            "Valuta": "Valuta prezzo",
                            "Controvalore_EUR": "Controvalore [EUR]",
                        }
                    )
                )
                st.markdown("### 🌍 Esposizione valutaria")
                st.dataframe(
                    _currency_summary,
                    use_container_width=True,
                    hide_index=True,
                ...
                _allocation = (
                    _portfolio_df.groupby("Asset_Class", as_index=False)["Peso_%"]
                    .sum()
                    .sort_values("Peso_%", ascending=False)
                )
                st.dataframe(_allocation, use_container_width=True, hide_index=True)

                st.markdown("### Target allocation")
                st.markdown("### 🎯 Metodo target allocation")

                _portfolio_profiles = {
                    "Personalizzato": {
                        "Azionario": 60.0,
                        "ETF": 0.0,
                        "Obbligazionario": 20.0,
                        "Oro/Commodities": 10.0,
                        "Crypto": 5.0,
                        "Strutturato": 5.0,
                    },
                    "Buffett 90/10": {
                        "Azionario": 90.0,
                        "ETF": 0.0,
                        "Obbligazionario": 10.0,
                        "Oro/Commodities": 0.0,
                        "Crypto": 0.0,
                        "Strutturato": 0.0,
                    },
                    "Bilanciato professionale": {
                        "Azionario": 55.0,
                        "ETF": 0.0,
                        "Obbligazionario": 25.0,
                        "Oro/Commodities": 10.0,
                        "Crypto": 0.0,
                        "Strutturato": 10.0,
                    },
                    "Growth diversificato": {
                        "Azionario": 60.0,
                        "ETF": 20.0,
                        "Obbligazionario": 10.0,
                        "Oro/Commodities": 5.0,
                    ...
                    "Innovation / high-conviction style": {
                        "Azionario": 65.0,
                        "ETF": 15.0,
                        "Obbligazionario": 10.0,
                        "Oro/Commodities": 5.0,
                ...
                _profile = st.selectbox(
                    ...
                    list(_portfolio_profiles),
                    key="portfolio_profile_v4511",
                )

                if _profile == "Buffett 90/10":
                    st.info(
                        "Modello semplice e aggressivo: 90% azionario "
                        "diversificato e 10% obbligazionario/liquidità. "
                        "Non è una raccomandazione personalizzata."
                    )
                elif _profile == "Innovation / high-conviction style":
                    st.warning(
                        "Modello aggressivo e concentrato sull'innovazione. "
                        "Può subire drawdown elevati e non rappresenta il "
                        "portafoglio ufficiale di ARK."
                    )

                _portfolio_targets = _portfolio_profiles[_profile].copy()

                _target_values = {}
                _target_cols = st.columns(3)
                for _index, (_asset, _default) in enumerate(_portfolio_targets.items()):
                    with _target_cols[_index % 3]:
                        _target_values[_asset] = st.number_input(
                            f"{_asset} (%)",
                            min_value=0.0,
                            max_value=100.0,
                            value=float(_default),
                            step=1.0,
                            key=f"portfolio_target_{_asset}",
                        )

                _target_total = sum(_target_values.values())
                if abs(_target_total - 100.0) > 0.01:
                    st.warning(
                        f"I target totalizzano {_target_total:.1f}%. "
                        "Portali al 100% prima di interpretare il riequilibrio."
                    )

                if _has_values:
                    _rebalance = build_rebalance(_portfolio_df, _target_values)
                    st.markdown("### Riequilibrio indicativo")
                    _rebalance_display = _rebalance.rename(
                        columns={
                            "Asset_Class": "Classe asset",
                            "Peso_attuale": "Peso attuale [%]",
                            "Target_%": "Target [%]",
                            "Scostamento_pp": "Scostamento [punti]",
                            "Valore_attuale": "Valore attuale [EUR]",
                            "Valore_target": "Valore target [EUR]",
                            "Importo_teorico": "Importo teorico [EUR]",
                        }
                    )
                    st.dataframe(
                        _rebalance_display,
                    ...
                    st.caption(
                        "Il riequilibrio usa esclusivamente i controvalori in EUR. "
                        "Importo teorico positivo = capitale da aggiungere; "
                        "negativo = esposizione teoricamente da ridurre. "
                        "Non è un ordine automatico."
                    )
                else:
                    st.info(
                        "Il riequilibrio quantitativo sara' disponibile quando "
                        "il file conterra' Quantita' e/o Valore di mercato."
                    )
                ...
                with _e1:
                    st.download_button(
                        "📄 Export CSV",
                        portfolio_to_csv(_portfolio_df),
                        "portafoglio_v45_11.csv",
                        "text/csv",
                        key="portfolio_csv_v4511",
                    )

                with _e2:
                    st.download_button(
                        "📊 Export XLSX",
                        portfolio_to_xlsx(_portfolio_df),
                        "portafoglio_v45_11.xlsx",
                        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key="portfolio_xlsx_v4511",
                    )

                with _e3:
                    st.download_button(
                        "📺 TradingView",
                        portfolio_to_tradingview(_portfolio_df),
                        "portafoglio_v45_11_tradingview.txt",
                        "text/plain",
                        key="portfolio_tv_v4511",
                    )

        except Exception as _portfolio_error:
            st.error(f"Errore import portafoglio: {_portfolio_error}")
...
# =========================================================================
st.markdown("---")
st.markdown('<div class="section-pill">💾 EXPORT PRO v45.11 — XLSX Multi-Sheet · TXT TradingView · Timestamp Auto</div>',unsafe_allow_html=True)
...
with ec1:
    st.download_button(
        ...
        f"TradingScanner_v45_11_Tutti_{_ts}.xlsx",
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        key="xlsx_all",
        help="Tutti i tab + sheet Summary con conteggi"
    )
with ec2:
    _tv_all = make_tv_sections(all_exp)
    st.download_button(
        "📥 TXT TV Tutti",
        data=_tv_all,
        file_name=f"TradingScanner_v45_11_Tutti_TradingView_{_ts}.txt",
        mime="text/plain",
        key="tv_txt_all",
        disabled=not bool(_tv_all),
        help="Formato TradingView a sezioni: un gruppo per riga e ticker separati da virgole.",
    )
with ec3:
    st.download_button(
        f"📊 XLSX {cur_tab}",
        _to_excel_pro({cur_tab: df_cur}),
        f"TradingScanner_v45_11_{cur_tab}_{_ts}.xlsx",
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        key="xlsx_curr"
    )
with ec4:
    if not df_cur.empty and "Ticker" in df_cur.columns:
        st.download_button(
            f"📥 TXT TV {cur_tab}",
            make_tv_txt(df_cur),
            f"TradingScanner_v45_11_{cur_tab}_TradingView_{_ts}.txt",
            "text/plain",
            key="tv_txt_curr"
        )
with ec5:
    ...
    if _df_ai2.empty:
        ...
        for _, _r in _df_ai2.head(60).iterrows():
            ...
            if st.session_state.get(_toggle_key, False):
                with st.expander(f"🤖 Modulo 2 — AI Analyst · {_tkr} ({_nome})", expanded=True):
                    _a2_row = _r.copy() if hasattr(_r, 'copy') else _r
                    _a2_nome  = _nome
                    _a2_stato = _stato or "—"
                    _a2_css   = _css if _css not in [None,""] else "—"
                    _a2_rsi   = _rsi if _rsi not in [None,""] else "—"
                    _a2_pr    = _a2_row.get("Prezzo","—")
                    _a2_atr   = _a2_row.get("ATR","—")
                    _a2_rs    = _a2_row.get("RS_20d","—")
                    _a2_volx  = _a2_row.get("Vol_Ratio", _a2_row.get("VolRatio","—"))
                    _a2_e20   = _a2_row.get("EMA20","—")
                    _a2_e50   = _a2_row.get("EMA50","—")
                    _a2_sq    = _a2_row.get("Squeeze","—")
                    _a2_early = _a2_row.get("Stato_Early","—")
                    try:
                        _s_gem = st.secrets.get("GEMINI_API_KEY","") or ""
                        _s_groq = st.secrets.get("GROQ_API_KEY","") or ""
                        _s_or  = st.secrets.get("OPENROUTER_API_KEY","") or ""
                        _s_ant = st.secrets.get("ANTHROPIC_API_KEY","") or ""
                    except Exception:
                        _s_gem = _s_groq = _s_or = _s_ant = ""
                    _a2_any_key = any([
                        _s_gem or st.session_state.get("_gemini_api_key",""),
                        _s_groq or st.session_state.get("_groq_api_key",""),
                        _s_or  or st.session_state.get("_openrouter_api_key",""),
                        _s_ant or st.session_state.get("_anthropic_api_key",""),
                    ])
                    _x1,_x2,_x3,_x4 = st.columns(4)
                    _x1.markdown(f"**Ticker:** `{_tkr}`")
                    _x2.markdown(f"**Nome:** {_a2_nome}")
                    _x3.markdown(f"**Stato:** {_a2_stato} | CSS: {_a2_css}")
                    _x4.markdown(f"**RSI:** {_a2_rsi} | Vol×: {_a2_volx}")
                    if not _a2_any_key:
                        st.warning("⚠️ Nessuna API key configurata.")
                        with st.expander("🔑 Configura API key qui (senza uscire dal tab)", expanded=True):
                            _q1, _q2 = st.columns(2)
                            with _q1:
                                _qg = st.text_input("Gemini API Key", type="password", placeholder="AIzaSy...", key=f"_q_gem_{_tkr}")
                                _qq = st.text_input("Groq API Key",   type="password", placeholder="gsk_...",   key=f"_q_groq_{_tkr}")
                            with _q2:
                                _qo = st.text_input("OpenRouter Key", type="password", placeholder="sk-or-...", key=f"_q_or_{_tkr}")
                                _qa = st.text_input("Claude Key",     type="password", placeholder="sk-ant-...", key=f"_q_ant_{_tkr}")
                            if st.button("💾 Salva e analizza", key=f"_q_save_{_tkr}", type="primary", use_container_width=True):
                                if _qg.strip(): st.session_state["_gemini_api_key"]     = _qg.strip()
                                if _qq.strip(): st.session_state["_groq_api_key"]       = _qq.strip()
                                if _qo.strip(): st.session_state["_openrouter_api_key"] = _qo.strip()
                                if _qa.strip(): st.session_state["_anthropic_api_key"]  = _qa.strip()
                                st.rerun()
                    else:
                        try:
                            _a2_rg = _get_market_regime()
                        ...
                        _a2_prompt = (
                            ...
                            f"SEGNALE: {_a2_stato} | CSS: {_a2_css}/100 | RSI: {_a2_rsi}" + _nl +
                            f"ATR: {_a2_atr} | Vol Ratio: {_a2_volx}x | RS vs SPY: {_a2_rs}%" + _nl +
                            f"EMA20: {_a2_e20} | EMA50: {_a2_e50} | Squeeze: {_a2_sq}" + _nl +
                            f"Stato Early: {_a2_early}" + _nl +
                            f"SCENARIO MACRO: {_a2_regime}" + _nl + _nl +
                            "Rispondi in italiano con questo formato ESATTO (max 2 righe per sezione):" + _nl + _nl +
                            "📊 SETUP:" + _nl + "[descrivi la struttura tecnica — trend, momentum, volume, squeeze]" + _nl + _nl +
                            "🎯 TARGET:" + _nl + "[T1 = entry + 1.5×ATR | T2 = entry + 3×ATR — valori numerici precisi]" + _nl + _nl +
                            "❌ INVALIDAZIONE:" + _nl + "[livello di prezzo che invalida il setup — stop loss ATR-based]" + _nl + _nl +
                            "⚠️ RISCHIO:" + _nl + "[rischio specifico principale in questo momento per questo titolo]"
                        )
                        _cache_key = f"_ai2_result_{_tkr}"
                        if _cache_key not in st.session_state:
                            with st.spinner(f"Analisi AI per {_tkr}..."):
                                try:
                                    st.session_state[_cache_key] = _ai_call_with_fallback(_a2_prompt)
                                except Exception as _a2_err:
                                    st.session_state[_cache_key] = ("ERROR:" + str(_a2_err)[:300], "—")
                        _a2_txt_out, _a2_prov_out = st.session_state.get(_cache_key, ("", "—"))
                        if str(_a2_txt_out).startswith("ERROR:"):
                            _em = str(_a2_txt_out)[6:]
                            st.error(f"❌ Errore: {_em[:220]}")
                        else:
                            st.markdown(
                                f"<div style='background:#0d1117;border:1px solid #1f2937;border-left:3px solid #0d9488;border-radius:0 8px 8px 0;padding:14px 18px;font-size:0.88rem;line-height:1.75'>{_a2_txt_out.replace(chr(10),'<br>')}</div>",
                                unsafe_allow_html=True)
                            st.caption(f"Provider: {_a2_prov_out}")
                    if st.button("✖ Chiudi analisi", key=f"ai2_close_{_tkr}"):
        ...
        # ── Analisi dettagliata inline (stessa logica del tab PRO) ──────────
        _sel_tkr = st.session_state.get("ai2_selected_ticker")
        if _sel_tkr:
            st.markdown("---")
            st.markdown(f"<div class='section-pill'>🤖 Modulo 2 — AI Analyst · {_sel_tkr} ({st.session_state.get('ai2_selected_name','')})</div>", unsafe_allow_html=True)

            # Recupera riga dati dal df sorgente o da df_ep come fallback
            _a2_row = pd.Series(dtype=object)
            for _a2_src in [_df_ai2, df_ep]:
                if not _a2_src.empty and "Ticker" in _a2_src.columns:
                    _a2_m = _a2_src[_a2_src["Ticker"] == _sel_tkr]
                    if not _a2_m.empty:
                        _a2_row = _a2_m.iloc[0]
                        break

            _a2_nome  = st.session_state.get("ai2_selected_name", str(_a2_row.get("Nome","")))
            _a2_stato = str(_a2_row.get("Stato_Pro", st.session_state.get("ai2_selected_setup","—")))
            _a2_css   = _a2_row.get("CSS","—")
            _a2_rsi   = _a2_row.get("RSI","—")
            _a2_pr    = _a2_row.get("Prezzo","—")
            _a2_atr   = _a2_row.get("ATR","—")
            _a2_rs    = _a2_row.get("RS_20d","—")
            _a2_volx  = _a2_row.get("Vol_Ratio","—")
            _a2_e20   = _a2_row.get("EMA20","—")
            _a2_e50   = _a2_row.get("EMA50","—")
            _a2_sq    = _a2_row.get("Squeeze","—")
            _a2_early = _a2_row.get("Stato_Early","—")
            ...
            # Info riepilogo ticker
            _a2c1, _a2c2, _a2c3, _a2c4 = st.columns(4)
            _a2c1.markdown(f"**Ticker:** `{_sel_tkr}`")
            _a2c2.markdown(f"**Nome:** {_a2_nome}")
            _a2c3.markdown(f"**Stato:** {_a2_stato} | CSS: {_a2_css}")
            _a2c4.markdown(f"**RSI:** {_a2_rsi} | Vol×: {_a2_volx}")

            if not _a2_any_key:
                st.warning("⚠️ Nessuna API key configurata. Apri il pannello 🔑 nel tab 🤖 AI Assistant.")
            else:
                # Regime macro
                try:
                    _a2_rg = _get_market_regime()
                ...
                _a2_nl = "\n"
                _a2_prompt = (
                    "Sei un analista tecnico professionista. Produci un brief operativo conciso." + _a2_nl + _a2_nl +
                    f"TICKER: {_sel_tkr} ({_a2_nome}) | PREZZO: {_a2_pr}" + _a2_nl +
                    f"SEGNALE: {_a2_stato} | CSS: {_a2_css}/100 | RSI: {_a2_rsi}" + _a2_nl +
                    f"ATR: {_a2_atr} | Vol Ratio: {_a2_volx}x | RS vs SPY: {_a2_rs}%" + _a2_nl +
                    f"EMA20: {_a2_e20} | EMA50: {_a2_e50} | Squeeze: {_a2_sq}" + _a2_nl +
                    f"Stato Early: {_a2_early}" + _a2_nl +
                    f"SCENARIO MACRO: {_a2_regime}" + _a2_nl + _a2_nl +
                    "Rispondi in italiano con questo formato ESATTO (max 2 righe per sezione):" + _a2_nl + _a2_nl +
                    "📊 SETUP:" + _a2_nl +
                    "[descrivi la struttura tecnica — trend, momentum, volume, squeeze]" + _a2_nl + _a2_nl +
                    "🎯 TARGET:" + _a2_nl +
                    "[T1 = entry + 1.5×ATR | T2 = entry + 3×ATR — valori numerici precisi]" + _a2_nl + _a2_nl +
                    "❌ INVALIDAZIONE:" + _a2_nl +
                    "[livello di prezzo che invalida il setup — stop loss ATR-based]" + _a2_nl + _a2_nl +
                    "⚠️ RISCHIO:" + _a2_nl +
                    "[rischio specifico principale in questo momento per questo titolo]"
                )

                # Esegui analisi solo al primo render dopo il rerun (evita re-call a ogni widget)
                _a2_cache_key = f"_ai2_result_{_sel_tkr}"
                if _a2_cache_key not in st.session_state:
                    with st.spinner(f"Analisi AI per {_sel_tkr}..."):
                        try:
                            _a2_txt, _a2_prov = _ai_call_with_fallback(_a2_prompt)
                            st.session_state[_a2_cache_key] = (_a2_txt, _a2_prov)
                        except Exception as _a2_err:
                            st.session_state[_a2_cache_key] = ("ERROR:" + str(_a2_err)[:300], "—")

                _a2_res = st.session_state.get(_a2_cache_key, ("", ""))
                _a2_txt_out, _a2_prov_out = _a2_res if isinstance(_a2_res, tuple) else (_a2_res, "—")

                if _a2_txt_out.startswith("ERROR:"):
                    _em = _a2_txt_out[6:]
                    if "NO_KEYS" in _em:
                        st.warning("⚠️ Nessuna API key — configura nel tab 🤖 AI Assistant.")
                    elif "ALL_FAILED" in _em:
                        st.error(f"❌ Tutti i provider falliti: {_em.replace('ALL_FAILED: ','')[:200]}")
                    else:
                        st.error(f"❌ Errore: {_em[:200]}")
                else:
                    st.markdown(
                        f"<div style='background:#0d1117;border:1px solid #1f2937;"
                        f"border-left:3px solid #0d9488;border-radius:0 8px 8px 0;"
                        f"padding:14px 18px;font-size:0.88rem;line-height:1.75'>"
                        f"{_a2_txt_out.replace(chr(10),'<br>')}</div>",
                        unsafe_allow_html=True)
                    st.caption(f"Provider: {_a2_prov_out}")

            _a2_close1, _a2_close2 = st.columns([1, 4])
            with _a2_close1:
                if st.button("✖ Chiudi analisi", key="ai2_close_analysis", use_container_width=True):
                    ...
                    # Pulisci cache risultato
                    for _k in list(st.session_state.keys()):
                        if _k.startswith("_ai2_result_"):
                            st.session_state.pop(_k, None)
                    st.rerun()
...
with tab_mom:
    ...
    # ── Sorgente ticker (identica logica Modulo 2 AI) ─────────────────────
    st.markdown("<div class='tab-legend'><strong>Sorgente ticker</strong> scegli da quale lista importare i ticker da analizzare <span class='sep'>•</span> I ticker vengono aggiunti a quelli manuali se presenti <span class='sep'>•</span> Max 40 ticker per scan</div>", unsafe_allow_html=True)

    _ms1, _ms2, _ms3 = st.columns([2, 2, 1])
    with _ms1:
        _ma_source = st.selectbox(
            "📂 Importa ticker da:",
            ["Manuale", "PRO/STRONG (scanner)", "EARLY (scanner)",
             "CONFLUENCE (scanner)", "REA-HOT (scanner)",
             "Serafini", "Finviz Pro", "Watchlist attiva", "Tutti i segnali"],
            key="ma_source_sel",
            index=st.session_state.get("ma_source_idx", 0)
        )
        st.session_state["ma_source_idx"] = ["Manuale","PRO/STRONG (scanner)","EARLY (scanner)",
            "CONFLUENCE (scanner)","REA-HOT (scanner)","Serafini","Finviz Pro",
            "Watchlist attiva","Tutti i segnali"].index(_ma_source)
    with _ms2:
        _ma_manual_in = st.text_input(
            "Ticker aggiuntivi / override manuale",
            value=st.session_state.get("ma_manual_cache", "SPY,QQQ,NVDA,TSLA,AAPL,AMD,META,GOOGL,MSFT,AMZN"),
            placeholder="es: AAPL,NVDA,MSFT  (separati da virgola)",
            key="ma_universe_manual").strip()
        st.session_state["ma_manual_cache"] = _ma_manual_in
    with _ms3:
        _ma_top_n = st.number_input("Max ticker", 5, 80, 30, 5, key="ma_top_n")

    # Costruisci lista ticker dalla sorgente selezionata
    _imported_tickers = []
    _import_info = ""
    _df_ma_src = pd.DataFrame()

    if _ma_source == "PRO/STRONG (scanner)" and not df_ep.empty and "Stato_Pro" in df_ep.columns:
        _df_ma_src = df_ep[df_ep["Stato_Pro"].isin(["PRO","STRONG"])].copy()
    elif _ma_source == "EARLY (scanner)" and not df_ep.empty and "Stato_Early" in df_ep.columns:
        _df_ma_src = df_ep[df_ep["Stato_Early"]=="EARLY"].copy()
    elif _ma_source == "CONFLUENCE (scanner)" and not df_ep.empty:
        if "Stato_Early" in df_ep.columns and "Stato_Pro" in df_ep.columns:
            _df_ma_src = df_ep[(df_ep["Stato_Early"]=="EARLY") & df_ep["Stato_Pro"].isin(["PRO","STRONG"])].copy()
    elif _ma_source == "REA-HOT (scanner)":
        if not df_rea.empty:
            _df_ma_src = df_rea.copy()
        elif not df_ep.empty and "Stato" in df_ep.columns:
            _df_ma_src = df_ep[df_ep["Stato"]=="HOT"].copy()
    elif _ma_source == "Serafini" and not df_ep.empty and "Ser_OK" in df_ep.columns:
        _df_ma_src = df_ep[df_ep["Ser_OK"].isin([True,"True","true"])].copy()
    elif _ma_source == "Finviz Pro" and not df_ep.empty and "FV_OK" in df_ep.columns:
        _df_ma_src = df_ep[df_ep["FV_OK"].isin([True,"True","true"])].copy()
    elif _ma_source == "Watchlist attiva":
        try:
            _wl_ma = load_watchlist()
            _active_list_ma = st.session_state.get("current_list_name", st.session_state.get("currentlistname","DEFAULT"))
            if not _wl_ma.empty and "list_name" in _wl_ma.columns:
                _df_ma_src = _wl_ma[_wl_ma["list_name"]==_active_list_ma].copy()
            elif not _wl_ma.empty:
                _df_ma_src = _wl_ma.copy()
        except Exception:
            _df_ma_src = pd.DataFrame()
    elif _ma_source == "Tutti i segnali" and not df_ep.empty:
        _df_ma_src = df_ep.copy()

    # Estrai ticker dalla sorgente
    if not _df_ma_src.empty:
        _tk_col = "Ticker" if "Ticker" in _df_ma_src.columns else ("ticker" if "ticker" in _df_ma_src.columns else None)
        if _tk_col:
            # Ordina per CSS/ProScore se disponibile
            for _sc in ["CSS","Pro_Score","Quality_Score","Early_Score"]:
                if _sc in _df_ma_src.columns:
                    _df_ma_src[_sc] = pd.to_numeric(_df_ma_src[_sc], errors="coerce").fillna(0)
                    _df_ma_src = _df_ma_src.sort_values(_sc, ascending=False)
                    break
            _imported_tickers = _df_ma_src[_tk_col].dropna().tolist()[:int(_ma_top_n)]
            _import_info = f"✅ **{len(_imported_tickers)}** ticker importati da **{_ma_source}**"

    # Unisci con input manuale (manuale ha precedenza se duplicati)
    _manual_tickers = [t.strip().upper() for t in _ma_manual_in.split(",") if t.strip()]
    if _ma_source == "Manuale":
        _final_tickers = _manual_tickers[:int(_ma_top_n)]
    else:
        # Importati + eventuali manuali aggiuntivi, deduplicati, max top_n
        _combined = list(dict.fromkeys(_imported_tickers + _manual_tickers))
        _final_tickers = _combined[:int(_ma_top_n)]

    _ma_universe = ",".join(_final_tickers) if _final_tickers else _ma_manual_in

    # Info box stato sorgente
    if _import_info:
        _imp_tickers_preview = ", ".join(_final_tickers[:8]) + ("..." if len(_final_tickers) > 8 else "")
        st.markdown(
            f"<div class='tab-legend'>{_import_info} <span class='sep'>→</span> "
            f"<span style='font-family:var(--font-mono);color:var(--text-primary)'>{_imp_tickers_preview}</span>"
            f"</div>", unsafe_allow_html=True)
    elif _ma_source != "Manuale" and _imported_tickers == []:
        st.warning(f"⚠️ Nessun ticker trovato in '{_ma_source}'. Avvia prima lo scanner o usa input manuale.")
    ...
    if _run_ma:
        ...
        _ma_tickers = [t.strip().upper() for t in _ma_universe.split(",") if t.strip()][:int(st.session_state.get("ma_top_n", 40))]
        _ma_new_alerts = []
    ...
    if st.session_state.mom_alerts:
        ...
        try:
            _ma_rg = _get_market_regime()
            _ma_regime_ctx = f"VIX={_ma_rg['vix']}, Regime={_ma_rg['regime']}, SPY 20d={_ma_rg['spy_mom_20d']:+.1f}%"
        except Exception:
            _ma_regime_ctx = "dati regime non disponibili"

        _mh = st.columns([1.2, 1.5, 1.5, 1, 0.8, 0.7, 0.7, 1, 1.2])
        for _hcol, _hlbl in zip(_mh, ["Ticker","Nome","Tipo","Valore","RSI","Vol×","CSS","Priorità","🧠 AI"]):
        ...
        for _ali, _al in enumerate(_ma_all[:50]):
            ...
            _al_tkr = _al['Ticker']
            _al_key = f"mom_ai_{_al_tkr}_{_ali}"

            _row_cols = st.columns([1.2, 1.5, 1.5, 1, 0.8, 0.7, 0.7, 1, 1.2])
            _row_cols[0].markdown(
                f"<a href='https://it.tradingview.com/chart/?symbol={_tv_s}' target='_blank' "
                f"style='text-decoration:none'><span style='font-family:Courier New;"
                f"color:#00ff88;font-weight:bold'>{_al_tkr}</span> "
                f"<span style='color:#2962ff;font-size:0.65rem'>↗</span></a>",
                unsafe_allow_html=True)
            ...
            with _row_cols[8]:
                if _ma_any_key:
                    if st.button("🧠 Analizza", key=_al_key, use_container_width=True):
                        st.session_state[f"_mom_ai_req_{_al_key}"] = not st.session_state.get(f"_mom_ai_req_{_al_key}", False)
                else:
                    st.markdown(
                        "<span style='color:#f59e0b;font-size:0.72rem'>"
                        "⚠️ Configurare nel tab PRO la sezione API KEYS"
                        "</span>",
                        unsafe_allow_html=True,
                    )

            # ── Analisi AI inline sotto la riga ────────────────────────────
            if st.session_state.get(f"_mom_ai_req_{_al_key}"):
                with st.expander(f"🤖 Modulo 2 — AI Analyst · {_al_tkr} ({_al.get('Nome','')})", expanded=True):
                    # Recupera dati dal df sorgente se disponibile
                    _ma_row = pd.Series()
                    for _src_df in [df_ep, _df_ma_src]:
                        if not _src_df.empty and "Ticker" in _src_df.columns:
                            _m = _src_df[_src_df["Ticker"]==_al_tkr]
                            if not _m.empty:
                                _ma_row = _m.iloc[0]
                                break
                    _ma_pr   = _ma_row.get("Prezzo","—") if not _ma_row.empty else "—"
                    _ma_atr  = _ma_row.get("ATR","—") if not _ma_row.empty else "—"
                    _ma_rs   = _ma_row.get("RS_20d","—") if not _ma_row.empty else "—"
                    _ma_e20  = _ma_row.get("EMA20","—") if not _ma_row.empty else "—"
                    _ma_e50  = _ma_row.get("EMA50","—") if not _ma_row.empty else "—"
                    _ma_sq   = _ma_row.get("Squeeze","—") if not _ma_row.empty else "—"
                    _ma_css  = _al.get("CSS","—")
                    _ma_rsi  = _al.get("RSI","—")
                    _ma_volx = _al.get("Vol×","—")
                    _ma_nome = _al.get("Nome","")
                    _ma_stato= _ma_row.get("Stato_Pro","—") if not _ma_row.empty else "—"

                    _nl = "\n"
                    _ma_prompt = (
                        "Sei un analista tecnico professionista. Produci un brief operativo conciso." + _nl + _nl +
                        f"TICKER: {_al_tkr} ({_ma_nome}) | PREZZO: {_ma_pr}" + _nl +
                        f"ALERT: {_ttype} | VALORE: {_al.get('Valore','—')} | PRIORITA': {_al.get('Priorita','—')}" + _nl +
                        f"CSS: {_ma_css}/100 | RSI: {_ma_rsi} | Vol Ratio: {_ma_volx}" + _nl +
                        f"ATR: {_ma_atr} | RS vs SPY: {_ma_rs}% | EMA20: {_ma_e20} | EMA50: {_ma_e50}" + _nl +
                        f"Squeeze: {_ma_sq} | Stato scanner: {_ma_stato}" + _nl +
                        f"SCENARIO MACRO: {_ma_regime_ctx}" + _nl + _nl +
                        "Rispondi in italiano con questo formato ESATTO (max 2 righe per sezione):" + _nl + _nl +
                        "📊 SETUP:" + _nl + "[descrivi struttura tecnica: trend, momentum, volume, squeeze]" + _nl + _nl +
                        "🎯 TARGET:" + _nl + "[T1 = entry + 1.5×ATR | T2 = entry + 3×ATR con valori numerici precisi]" + _nl + _nl +
                        "❌ INVALIDAZIONE:" + _nl + "[livello di prezzo che invalida il setup - stop loss ATR-based]" + _nl + _nl +
                        "⚠️ RISCHIO:" + _nl + "[rischio specifico principale per questo titolo ora]"
                    )
                    with st.spinner(f"Analisi AI per {_al_tkr}..."):
                        try:
                            _ma_txt, _ma_prov = _ai_call_with_fallback(_ma_prompt)
                            st.markdown(
                                f"<div style='background:#0d1117;border:1px solid #1f2937;"
                                f"border-left:3px solid #0d9488;border-radius:0 8px 8px 0;"
                                f"padding:12px 16px;font-size:0.88rem;line-height:1.7'>"
                                f"{_ma_txt.replace(chr(10),'<br>')}</div>",
                                unsafe_allow_html=True)
                            st.caption(f"Provider: {_ma_prov}")
                        except Exception as _ma_err:
                            _ma_em = str(_ma_err)
                            if "NO_KEYS" in _ma_em:
                                st.warning("⚠️ Nessuna API key — configura nel tab 🤖 AI Assistant.")
                            elif "ALL_FAILED" in _ma_em:
                                st.error(f"❌ Tutti i provider falliti: {_ma_em.replace('ALL_FAILED: ','')[:200]}")
                            else:
                                st.error(f"❌ Errore: {_ma_em[:200]}")
                    if st.button("✕ Chiudi analisi", key=f"mom_ai_close_{_al_key}"):
                        st.session_state.pop(f"_mom_ai_req_{_al_key}", None)
                        st.rerun()
