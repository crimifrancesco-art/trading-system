...
# PAGE CONFIG
# =========================================================================
st.set_page_config(page_title="Trading Scanner PRO 50.0",layout="wide",page_icon="🧠")
...
<script>
document.getElementById('_btt').addEventListener('click', function(){ try{window.parent.location.hash='top-page';}catch(e){}
  // Cerca il contenitore scrollabile in tutti i frame accessibili
  function scrollUp(doc){
    if(!doc) return;
    // Streamlit >= 1.28: main block container
    var sel=[
    ...
    sel.forEach(function(s){
      ...
      if(el && el.scrollTop>0){ el.scrollTop=0; try{el.scrollTo({top:0,behavior:'smooth'});}catch(e){} }
    });
    // Walk tutti gli elementi scrollati
    try{
      var all=doc.getElementsByTagName('*');
      for(var i=0;i<all.length;i++){
        if(all[i].scrollTop>10){
          all[i].scrollTop=0;
          try{all[i].scrollTo({top:0,behavior:'smooth'});}catch(e){}
        }
      }
    }catch(e){}
    try{doc.defaultView && doc.defaultView.scrollTo({top:0,behavior:'smooth'});}catch(e){}
  }
  // Questo componente vive in un iframe: risale al parent
  try{ scrollUp(window.parent.document); }catch(e){}
  try{ scrollUp(window.parent.parent.document); }catch(e){}
  try{ scrollUp(document); }catch(e){}
});
</script>
""", height=60)
st.markdown("# 🧠 Trading Scanner PRO 50.0")
...
V50_RELEASE_NOTES = {
    ...
    "focus": [
        "guided scanner workflow",
        "lighter sidebar",
        "clearer hierarchy",
        "consistent top-N outputs"
    ]
}
...
def render_v50_workflow_header():
    try:
        ...
        chips = ''.join([f"<span style=\'padding:6px 10px;border-radius:999px;background:#12343b;color:#d9fffb;border:1px solid #1f5e69;font-size:0.82rem\'>{s}</span>" for s in steps])
        st.markdown(f"<div style='display:flex;flex-wrap:wrap;gap:8px;margin:6px 0 14px 0'>{chips}</div>", unsafe_allow_html=True)
    except Exception:
        pass
...
def render_v50_focus_box():
    try:
        mkts = []
        for key, name in [('mSP500','S&P 500'), ('mNasdaq','Nasdaq 100'), ('mFTSE','FTSE MIB'), ('mEurostoxx','Eurostoxx 600'), ('mDow','Dow Jones'), ('mRussell','Russell 2000'), ('mStoxxEmerging','Stoxx Emerging 50'), ('mUSSmallCap','US Small Cap 2000')]:
            if bool(st.session_state.get(key, False)):
                mkts.append(name)
        markets_txt = ', '.join(mkts[:4]) + (f' +{len(mkts)-4}' if len(mkts) > 4 else '') if mkts else 'Nessun mercato selezionato'
        strong = 'STRONG only' if bool(st.session_state.get('showstrongonly', st.session_state.get('show_strong_only', False))) else 'Bilanciato'
        topn = int(st.session_state.get('top', 15))
        early = float(st.session_state.get('minearlyscore', st.session_state.get('min_early_score', 0)))
        qual = int(st.session_state.get('minquality', st.session_state.get('min_quality', 0)))
        pro = float(st.session_state.get('minproscore', st.session_state.get('min_pro_score', 0)))
        st.info(f"Workflow attivo → Mercati: {markets_txt} | Profilo: {strong} | Soglie: Early≥{early} · Quality≥{qual} · Pro≥{pro} | Top N: {topn}")
    except Exception:
        pass

st.sidebar.title("⚙️ Configurazione V50")
...
with st.sidebar.expander("2️⃣ Profilo Scanner",expanded=True):
...
with st.sidebar.expander("1️⃣ Mercati",expanded=True):
