...
[data-testid="stMetricLabel"]{color:#787b86 !important;font-size:0.85rem !important;}
[data-testid="stMetricValue"]{color:#26a69a !important;font-size:1.75rem !important;
...
.ag-header-cell-label{color:#50c4e0 !important;font-family:'Trebuchet MS',sans-serif !important;
    font-size:0.89rem !important;letter-spacing:0.5px;text-transform:uppercase;}
...
.section-pill{display:inline-block;background:#1e222d;
    ...
    font-family:'Trebuchet MS',sans-serif;font-size:0.92rem;color:#50c4e0;
...
.wl-card-name{color:#8b949e;font-size:0.92rem;margin-top:2px;}
.wl-card-badge{display:inline-block;border-radius:10px;padding:2px 8px;font-size:0.83rem;font-weight:bold;margin-right:4px;}
...
.legend-table{width:100%;border-collapse:collapse;font-family:'Courier New',monospace;font-size:0.92rem;}
...
@media(max-width:480px){
 ...
 [data-testid="stTabs"]>div:first-child>button{font-size:0.76rem!important;padding:3px 5px!important}
 [data-testid="stButton"]>button{min-height:44px!important}
}
@media(min-width:481px) and (max-width:768px){
 [data-testid="stMetricValue"]{font-size:1.35rem!important}
 [data-testid="stTabs"]>div:first-child>button{font-size:0.79rem!important;padding:4px 6px!important}
}
</style>
"""
...
this.eGui.style.cssText='color:'+m.c+';background:'+m.bg+';padding:1px 5px;'
  +'border-radius:3px;font-size:0.88rem;font-family:Courier New;';
}getGui(){return this.eGui;}}""")
...
lbl.style.cssText='font-family:Courier New;font-size:0.89rem;font-weight:bold;color:'+col+';min-width:32px;text-align:right;';
...
lbl.style.cssText='font-family:Courier New;font-size:0.89rem;color:'+col+';min-width:24px;';
this.eGui.appendChild(lbl);this.eGui.appendChild(bar);
}getGui(){return this.eGui;}}""")
...
lbl.style.cssText='font-family:Courier New;font-size:0.88rem;color:'+col+';min-width:44px;';
this.eGui.appendChild(lbl);this.eGui.appendChild(bar);
}getGui(){return this.eGui;}}""")
...
this.eGui.innerText=v;
this.eGui.style.cssText='color:'+col+';font-family:Courier New;font-weight:bold;font-size:0.92rem;';
}getGui(){return this.eGui;}}""")
...
# ── v41h: Backup/Restore watchlist su file JSON locale ───────────────────────
...
if not _df_ep_live.empty:
    ...
    st.sidebar.markdown(
        ...
        f"padding:8px 12px;margin:6px 0;font-family:Courier New;font-size:0.92rem;'>"
...
st.sidebar.markdown(
    ...
    f"border-radius:0 4px 4px 0;padding:5px 10px;font-size:0.83rem;margin:4px 0'>"
...
{rows}</table><br><span style="color:#6b7280;font-size:0.88rem">
...
def render_scan_tab(df,status_filter,sort_cols,ascending,title):
    ...
    if st.button(f"➕ Aggiungi a '{st.session_state.current_list_name}'",key=f"btn_{title}"):
        if not selected_df.empty and "Ticker" in selected_df.columns:
            ...
            _wl_save_backup()  # v41h: backup locale immediato
...
# TABS
# =========================================================================
# ── v41h: Menu sticky multi-riga — tutti i tab visibili su 3 righe ──────
st.markdown("""<style>
/* Sticky tab bar */
[data-testid="stTabs"] {
...
def _render_pattern_alerts_v41(df_src, tab_name="x"):
    ...
    for ar in sorted(_rows,key=lambda x:len(x["Pattern"]),reverse=True)[:30]:
        ...
        _ac1.markdown(f"<span style='font-family:Courier New;color:{_sc};font-weight:bold'>{ar['Ticker']}</span><br><span style='color:#6b7280;font-size:0.83rem'>{ar['Nome']}</span>",unsafe_allow_html=True)
        _ac2.markdown(f"<span style='font-family:Courier New;font-size:0.92rem'>${ar['Prezzo']}</span><br><span style='color:#787b86;font-size:0.83rem'>RSI {ar['RSI']} · CSS {ar['CSS']}</span>",unsafe_allow_html=True)
        _parts=[]
        for p in ar["Pattern"]:
            ...
            _parts.append(f"<span style='background:{_bg};color:{_tx};border-radius:3px;padding:1px 6px;font-size:0.83rem;margin-right:3px'>{_PATTERN_ALERTS_V39.get(p,{}).get('icon','🔔')} {_PATTERN_ALERTS_V39.get(p,{}).get('label',p)}</span>")
...
def _render_news_v41(df_ep_news):
    ...
    for n in _news[:40]:
        ...
        _nome_n_html = (f" <span style='color:#6b7280;font-size:0.82rem;font-style:italic'>{_nome_n}</span>" if _nome_n else '')
        _c1.markdown(
        ...
        _c2.markdown(f"<span style='color:{_sc2};font-size:0.88rem'>{n['Sentiment']}</span>",unsafe_allow_html=True)
        _c3.markdown(f"<a href='{n['Link']}' target='_blank' style='color:#b2b5be;font-size:0.92rem;text-decoration:none'>{n['Titolo']}</a> <span style='color:#374151;font-size:0.82rem'>{n['Data']}</span>",unsafe_allow_html=True)
...
def _render_ai_explainer_v41(df_source, tab_name="PRO"):
    ...
    with st.expander(
        ...
        st.markdown(
            ...
            "padding:10px 14px;margin-bottom:10px;font-size:0.90rem;color:#b2b5be'>"
    ...
    for _col, _lbl in zip(_ai_cols, ["Ticker","Stato","CSS","RSI","Modulo 2"]):
        _col.markdown(f"<span style='color:#50c4e0;font-size:0.88rem;font-weight:bold;"
    ...
    for _, _row_ai in _df_ai.iterrows():
        ...
        _ac1.markdown(
            ...
            f"{_tkr_ai}</span></a><br>"
            f"<span style='color:#787b86;font-size:0.83rem'>{_nome_ai}</span>",
            unsafe_allow_html=True)
        _ac2.markdown(f"<span style='color:{_sc};font-weight:bold;font-size:0.92rem'>"
                      f"{_stato_ai}</span>", unsafe_allow_html=True)
        _ac3.markdown(
            f"<span style='font-family:Courier New;font-size:0.92rem;"
                ...
                with st.spinner(f"Analisi {_tkr_ai}..."):
                    try:
                        ...
                        st.markdown(
                            ...
                            f"padding:12px 16px;font-size:0.97rem;line-height:1.6'>"
...
[data-testid="stTabs"] [data-baseweb="tab"] { white-space:nowrap !important; font-size:0.84rem !important; padding:4px 8px !important; flex-shrink:0 !important; margin-bottom:2px !important; max-width:160px !important; border-radius:4px 4px 0 0 !important; border:1px solid #2a2e3966 !important;}
...
with tab_home:
    # ── v41h — ALERT BANNER: segnali STRONG visibili subito ──────────────
    try:
        ...
        if not _df_ep_banner.empty and "Stato_Pro" in _df_ep_banner.columns:
            ...
            if _strong_count > 0:
                ...
                _banner_tickers_html = "  ".join(
                    f"<a href='https://it.tradingview.com/chart/?symbol={t.replace('.MI','%3AMI')}' target='_blank' style='color:#ffd700;font-family:Courier New;font-weight:bold;text-decoration:none;font-size:0.97rem'>{t}</a>"
                    f"<span style='color:#9ca3af;font-size:0.82rem;font-style:italic'>{str(_df_strong_map.get(t,''))[:18]}</span>"
                ...
                st.markdown(
                    ...
                    f"<span style='color:#ffd700;font-weight:bold;font-size:0.92rem;"
                    f"white-space:nowrap'>★ STRONG ({_strong_count})</span>"
                    f"<span style='color:#6b7280;font-size:0.88rem'>{_banner_tickers_html}{_banner_extra}</span>"
                    f"<span style='color:#374151;font-size:0.83rem;margin-left:auto'>"
                ...
                st.markdown(
                    ...
                    f"font-size:0.90rem;color:#00ff88'>✦ {_pro_count} segnali PRO attivi — nessun STRONG</div>",
                    unsafe_allow_html=True
                )
    except Exception:
        pass
    ...
    try:
        ...
        _regime_badge_html = (
            ...
            f"<span style='color:#787b86;font-size:0.92rem;margin-left:16px'>"
        ...
        if _rn in ("Crisis", "Risk-Off"):
            _regime_badge_html += (
                ...
                f"font-size:0.88rem;font-weight:bold'>⚠️ Segnali deboli soppressi</span>"
            )
    ...
    # ── v41h — MERCATI LIVE GLOBALE con mini sparkline 5gg ─────────────────
    ...
    try:
        ...
        if _live_data_v41:
            ...
            _live_html = (
                ...
                f"<span style='color:#2962ff;font-weight:bold;font-size:1.00rem;"
                f"letter-spacing:1.5px'>📊 MERCATI LIVE</span></a>"
                f"<span style='color:#6b7280;font-size:0.73rem;margin-left:12px'>{_now_str}</span>"
                f"<span style='color:#374151;font-size:0.82rem;margin-left:10px'>· aggiorna ogni 60s · YTD = performance da inizio anno</span>"
                f"</div>"
            )
            ...
            for _grp_name, _syms in _groups:
                ...
                _live_html += (
                    f"<div style='margin-bottom:12px'>"
                    f"<div style='color:#50c4e0;font-size:0.79rem;font-weight:bold;"
                ...
                for _sym in _avail:
                    ...
                    if _ytd is not None:
                        ...
                        _ytd_html = (
                            f"<div style='color:{_ytd_c};font-size:0.79rem;"
                            f"margin-top:3px;font-weight:500'>"
                            f"YTD {_ytd_ar}{abs(_ytd):.1f}%</div>"
                        )
                    else:
                        _ytd_html = "<div style='color:#374151;font-size:0.79rem;margin-top:3px'>YTD —</div>"
                    ...
                    _live_html += (
                        ...
                        f"<div style='color:#9ca3af;font-size:0.78rem;white-space:nowrap;"
    ...
    # ── v41h FEATURE 5 — MAPPA CALORE GLOBALE SVG ─────────────────────────
    with st.expander("🌍 Mappa Calore Globale — Performance indici mondiali v41h", expanded=True):
        try:
            ...
            if _live_for_map:
                ...
                def _map_card_v41h(lbl, sym):
                    ...
                    return (f"<div style='background:{_bg};border:1px solid #2a2e39;border-radius:6px;"
                            ...
                            f"<div style='color:#e2e8f0;font-size:0.76rem;font-weight:bold'>{lbl}</div>"
                            f"<div style='color:{_cl};font-family:Courier New;font-weight:bold;font-size:0.94rem'>{_ar}{abs(_c):.1f}%</div>"
                            f"<div style='color:#9ca3af;font-size:0.76rem'>{_ps}</div></div>")
                # v41h: layout HTML flex puro - nessuna colonna Streamlit fissa
                _map_html = (
    ...
    # v41h: correlazioni rimosse dalla Home (già nel tab Settori)
        ...
        if _earn_data:
            ...
            for _ed in _earn_data[:25]:
                ...
                _ea.markdown(
                    ...
                    f"<span style='color:#2962ff;font-size:0.77rem'> ↗</span></a>"
                    f"<br><span style='color:#787b86;font-size:0.83rem'>{_nome_ed}</span>",
                    unsafe_allow_html=True)
                _eb.markdown(
                    f"<span style='color:#d1d4dc;font-size:0.85rem'>{_ed['Earnings Date']}</span>",
                    unsafe_allow_html=True)
                _ec_col.markdown(
                    f"<b style='font-size:0.88rem;color:{_ed['_color']}'>{_ed['Giorni']:+d}gg</b>",
                    unsafe_allow_html=True)
                _edd.markdown(
                    f"<span style='background:{_ed['_color']}22;color:{_ed['_color']};"
                    f"border:1px solid {_ed['_color']}44;border-radius:4px;"
                    f"padding:1px 8px;font-size:0.85rem;font-weight:bold'>"
                    f"{_ed['Badge']}</span>",
                    unsafe_allow_html=True)
        else:
    ...
    st.markdown('---')
    with st.expander('📰 NEWS & SENTIMENT v41h — Ultime news con score sentiment · TradingView Italia', expanded=False):
        _render_news_v41(df_ep)
    st.markdown('---')
    # ── v41h FEATURE 3 — HEATMAP SETTORIALE LIVE ──────────────────────────
    with st.expander("🌡️ Heatmap Settoriale Live — 11 settori × 3 periodi", expanded=True):
        @st.cache_data(ttl=300, show_spinner=False)
        def _sector_heatmap_home_v41h():
        ...
        _ht_period = st.radio("Periodo heatmap", ["1d","1m","3m"], index=0,
                               ...
                               label_visibility="collapsed")
        try:
            _ht_data = _sector_heatmap_home_v41h()
            if _ht_data:
                ...
                for _sec_lbl, _sec_d in _ht_sorted:
                    ...
                    _ht_html += (
                        ...
                        f"<div style='color:#fff;font-size:0.83rem;font-weight:bold;"
                        f"margin-bottom:2px'>{_sec_lbl}</div>"
                        f"<div style='color:#fff;font-family:Courier New;font-size:0.92rem;"
    ...
    # ── v41h: Scanner Summary — segnali rapidi visibili dalla home ──────────
    if not df_ep.empty:
        st.markdown("---")
        ...
        with _hs_c1:
            ...
            if "Stato_Pro" in df_ep.columns and "CSS" in df_ep.columns:
                ...
                for _, _r in _top_pro.iterrows():
                    ...
                    _nome_lbl=f" <span style='color:#9ca3af;font-size:0.82rem;font-style:italic'>{_nome_pr}</span>" if _nome_pr else ''
                    st.markdown(
                        ...
                        f"{_nome_lbl}"
                        f"<span style='color:#6b7280;font-size:0.83rem'> · CSS {_r.get('CSS','—')} · {_r.get('Stato_Pro','')}</span>",
                        unsafe_allow_html=True)
            else:
        ...
        with _hs_c2:
            ...
            if "Stato_Early" in df_ep.columns and "Early_Score" in df_ep.columns:
                ...
                for _, _r in _top_early.iterrows():
                    ...
                    _nome_ea_lbl = (f" <span style='color:#9ca3af;font-size:0.82rem;font-style:italic'>{_nome_ea}</span>" if _nome_ea else '')
                    st.markdown(
                        f"<a href='https://it.tradingview.com/chart/?symbol={_tv}' target='_blank' "
                        f"style='text-decoration:none'>"
                        ...
                        f"{_r.get('Ticker','')}</span></a>"
                        f"{_nome_ea_lbl}"
                        f"<span style='color:#6b7280;font-size:0.83rem'> · E:{_r.get('Early_Score','—')} · RSI {_r.get('RSI','—')}</span>",
                        unsafe_allow_html=True)
            else:
                st.caption("Avvia lo scanner")
        ...
        with _hs_c3:
            ...
            if not df_rea.empty and "Vol_Ratio" in df_rea.columns:
                ...
                for _, _r in _top_hot.iterrows():
                    ...
                    _nome_hot_lbl = (f" <span style='color:#9ca3af;font-size:0.82rem;font-style:italic'>{_nome_hot}</span>" if _nome_hot else '')
                    st.markdown(
                        f"<a href='https://it.tradingview.com/chart/?symbol={_tv}' target='_blank' "
                        f"style='text-decoration:none'>"
                        ...
                        f"{_r.get('Ticker','')}</span></a>"
                        f"{_nome_hot_lbl}"
                        f"<span style='color:#6b7280;font-size:0.83rem'> · Vol×{_vr}</span>",
                        unsafe_allow_html=True)
            else:
                st.caption("Avvia lo scanner")

    # ── v41h: Suggerimenti ────────────────────────────────
    with st.expander('💡 Suggerimenti v41h — Novità e roadmap', expanded=False):
        st.markdown("""
**✅ Implementato in v41h:**
...
- 📱 Layout mobile-first con CSS container queries
    ...
    # ── v41h FEATURE 4 — Portfolio P&L dalla Watchlist ──────────────────────
    try:
        ...
        if _pnl_positions_home:
            ...
            if _pnl_rows_h:
                ...
                st.markdown(
                    ...
                    "<table style='width:100%;border-collapse:collapse;font-size:0.92rem;background:#131722'>"
    ...
    st.markdown('<div class="section-pill">🗓️ MACRO CALENDAR v41h — Fed · CPI · NFP · PCE</div>', unsafe_allow_html=True)
    ...
    if _mc39:
        ...
        for _i39,_ev41 in enumerate(_mc39[:4]):
            ...
            _mc39c[_i39].markdown(f"<div style='background:#1e222d;border-top:2px solid {_ic39};border-radius:0 0 6px 6px;padding:8px 10px'><div style='color:{_ic39};font-size:0.82rem'>{_ev41['Impatto']} · {_ev41['Giorni']}gg</div><div style='color:#d1d4dc;font-size:0.92rem;font-weight:bold'>{_ev41['Evento']}</div><div style='color:#6b7280;font-size:0.82rem'>{_ev41['Data']}</div></div>",unsafe_allow_html=True)
    with st.expander('📅 Calendario completo 90 giorni',expanded=False):
        for _ev41 in _macro_ev41[:20]:
            ...
            st.markdown(f"<div style='border-left:3px solid {_ic392};padding:4px 10px;margin:2px 0'><span style='color:{_ic392};font-size:0.85rem'>{_ev41['Data']}</span> <b style='color:#d1d4dc'>{_ev41['Evento']}</b> <span style='color:{_ic392};float:right'>{_dot39} {_ev41['Giorni']}gg</span></div>",unsafe_allow_html=True)
...
with tab_r:
    ...
    if not df_rea.empty:
        ...
        st.markdown(
            ...
            f'padding:8px 14px;border-radius:0 4px 4px 0;margin:8px 0;font-size:0.90rem">'
...
with tab_mtf:
    ...
    _badge_html = "".join(
        ...
        f'font-family:Courier New;font-size:0.88rem">' 
        f'<b style="color:{c}">{t}</b>' 
        f'<span style="color:#b2b5be;margin-left:5px">{n}</span>' 
        f'<span style="color:#5a6478;margin-left:5px;font-size:0.82rem">{cap}</span>' 
        f'</span>'
...
<span style="color:#6b7280;font-size:0.92rem">{_n_cats} categorie — {_n_assets} asset totali</span>
</div>
""", unsafe_allow_html=True)
    ...
    with _hc1:
        if _crisis_df_cached is not None:
            ...
            st.markdown(
                ...
                f'border-radius:6px;padding:8px 12px;font-size:0.92rem">'
            ...
            st.markdown(
                ...
                'border-radius:6px;padding:8px 12px;font-size:0.92rem">'
...
with tab_w:
    ...
    with st.expander("📈 P&L Tracker & Alert Engine v41h", expanded=False):
        ...
        with _pnl_col2:
            ...
            # ── v41h auto-notifica Telegram su FIRED ───────────────
                ...
                for _fa in [a for a in _alerts.values() if a.get('fired') and not a.get('notified')]:
                    try:
                        ...
                        _rqf.post(f'https://api.telegram.org/bot{_tgtv}/sendMessage',
                            json={'chat_id':_cgpv,'text':f"🚨 ALERT v41h\n{_fa['tkr']} {_fa['type']} {_fa['val']}"},timeout=5)
                        _fa['notified']=True
                    except Exception: pass

        # ── v41h pannello notifiche ─────────────────────────────
        with st.expander('📡 Canali Notifica v41h', expanded=False):
                ...
                if st.button('📨 Report P&L Telegram',key='pnl_tg_send'):
                    try:
                        ...
                        _rqt.post(f'https://api.telegram.org/bot{_tgtok}/sendMessage',
                            json={'chat_id':_tgcht,'text':'💼 Dashboard v41h P&L','parse_mode':'Markdown'},timeout=8)
                        st.success('✅ Inviato!')
                ...
                if st.button('🔔 Test Telegram',key='pnl_tg_test'):
                    try:
                        ...
                        _rqt2.post(f'https://api.telegram.org/bot{_tgtok}/sendMessage',
                            json={'chat_id':_tgcht,'text':'✅ Test OK v41h'},timeout=5)
            ...
            with _nc2:
                ...
                if st.button('📧 Report P&L Email',key='pnl_email_send'):
                    try:
                        ...
                        _m=_MT('Dashboard v41h P&L'); _m['Subject']='P&L v41h'; _m['From']=_emfrm; _m['To']=_emto
    ...
    with _wl_col1:
        if _GH_SYNC:
            ...
            st.markdown(
                ...
                f'padding:6px 12px;border-radius:0 4px 4px 0;font-size:0.92rem;">'
            ...
            st.markdown(
                ...
                'padding:6px 12px;border-radius:0 4px 4px 0;font-size:0.92rem;">'
  ...
  <div style="display:flex;justify-content:space-between;align-items:center">
    ...
    <div style="color:#374151;font-size:0.83rem">{origine} · {str(created)[:10]}</div>
  </div>
    ...
    if not _mtf_tickers:
        ...
        if _mtf_res:
            ...
            for _r in _mtf_res:
                ...
                _nome_disp = (f"<span style='color:#787b86;font-size:0.83rem;display:block'>{_r['Nome']}</span>"
                              if _r["Nome"] else "")
                _rc[0].markdown(
...
with tab_journal:
    ...
    if _journal:
        ...
        for _, _row_j in _df_j_disp.iterrows():
            ...
            _rj1.markdown(f"<span style='color:#6b7280;font-size:0.85rem'>#{int(_row_j['ID'])}</span>",
                          unsafe_allow_html=True)
            _rj2.markdown(f"<b style='font-family:Courier New;color:#00ff88'>{_row_j['Ticker']}</b>",
                          unsafe_allow_html=True)
            _rj3.markdown(f"<span style='font-size:0.88rem;color:#58a6ff'>{_row_j['Setup']}</span>",
                          unsafe_allow_html=True)
            _rj4.markdown(f"<span style='font-family:Courier New;font-size:0.92rem'>"
                          f"${_row_j['Entry $']:.2f}</span>", unsafe_allow_html=True)
            _exit_display = "Open" if _is_open else f"${float(_row_j['Exit $']):.2f}"
            _rj5.markdown(f"<span style='font-family:Courier New;font-size:0.92rem'>"
            ...
            _rj6.markdown(f"<b style='color:{_pnl_color};font-family:Courier New'>"
                          f"{_pnl_display}</b>",
                          unsafe_allow_html=True)
            _rj7.markdown(f"<span style='color:#6b7280;font-size:0.88rem'>{_row_j['Note']}</span>",
                          unsafe_allow_html=True)
...
with tab_regime:
  ...
  <text x="155" y="113" font-size="8" fill="#00ff88">GREED</text>
        ...
        with _fg_col2:
            ...
            st.markdown(
                f"<div style='margin-bottom:12px'>"
                f"<span style='color:#787b86;font-size:0.85rem'>REGIME SCORE: "
        ...
        for _rt, _rtxt in _pb:
            st.markdown(
                ...
                f"<span style='color:{_pb_c};font-weight:bold;min-width:115px;font-size:0.92rem'>{_rt}</span>"
                f"<span style='color:#d1d4dc;font-size:0.92rem'>{_rtxt}</span></div>",
                unsafe_allow_html=True)
    ...
    if not _sr_df.empty:
        ...
        # v41h: heatmap gia’ presente nella Home — bar chart disabilitato
        # st.plotly_chart(_fig_sr, use_container_width=True, key="sector_heatmap_v41")  # disabled v41h
        ...
        for _i, _rk_row in _sr_rank.iterrows():
            ...
            _rk2.markdown(f"<b>{_rk_row['Sector']}</b> <span style='color:#6b7280;font-size:0.88rem'>"
            ...
            for _col_rk, _p in zip([_rk3,_rk4,_rk5,_rk6,_rk7,_rk8], _periods_sr):
                ...
                _col_rk.markdown(f"<span style='font-family:Courier New;color:{_c};"
                                  f"font-size:0.92rem'>{_v:+.1f}%</span>",
                                  unsafe_allow_html=True)
    else:
    ...
    if _ps_entry > 0 and _ps_stop > 0 and _ps_stop < _ps_entry:
        ...
        st.markdown(
            ...
            f"margin-top:8px;font-family:Courier New;font-size:0.94rem'>"
            ...
            try:
                ...
                for _sec_nm, _cnt in _sec_sorted:
                    ...
                    st.markdown(
                        ...
                        f"<span style='font-family:Courier New;font-size:0.92rem;min-width:160px;"
                        ...
                        f"<span style='font-family:Courier New;font-size:0.92rem;color:{_c_sec};"
                ...
                with st.spinner("Analisi AI..."):
                    try:
                        ...
                        st.markdown(
                            ...
                            f"padding:14px 18px;font-size:0.97rem;line-height:1.7'>"
...
# =========================================================================
with tab_mom:
    st.markdown('<div class="section-pill">⚡ MOMENTUM ALERTS v41h — Ticker · Nome · Tipo · Valore · RSI · Vol× · CSS · Priorità</div>',
                unsafe_allow_html=True)
    ...
    if st.session_state.mom_alerts:
        ...
        for _hcol, _hlbl in zip(_mh, ["Ticker","Nome","Tipo","Valore","RSI","Vol×","CSS","Priorità"]):
            _hcol.markdown(f"<span style='color:#50c4e0;font-size:0.83rem;font-weight:bold;"
        ...
        for _al in _ma_all[:50]:
            ...
            _row_cols[1].markdown(
                f"<span style='color:#787b86;font-size:0.88rem'>{_al.get('Nome','')}</span>",
                unsafe_allow_html=True)
            _row_cols[2].markdown(
                f"<span style='color:{_tcolor};font-size:0.90rem;font-weight:bold'>{_ttype}</span>",
                unsafe_allow_html=True)
            _row_cols[3].markdown(
                f"<span style='font-family:Courier New;color:#d1d4dc;font-size:0.92rem'>"
                f"{_al.get('Valore','')}</span>", unsafe_allow_html=True)
            _row_cols[4].markdown(
                f"<span style='font-family:Courier New;color:#b2b5be;font-size:0.88rem'>"
            ...
            _row_cols[7].markdown(
                ...
                f"font-size:0.83rem;font-weight:bold'>{_al.get('Priorità','')}</span>",
                unsafe_allow_html=True)
