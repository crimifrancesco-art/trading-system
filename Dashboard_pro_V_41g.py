...
# ── v41g: Backup/Restore watchlist su file JSON locale ───────────────────────
...
def render_scan_tab(df,status_filter,sort_cols,ascending,title):
    ...
    if st.button(f"➕ Aggiungi a '{st.session_state.current_list_name}'",key=f"btn_{title}"):
        if not selected_df.empty and "Ticker" in selected_df.columns:
            ...
            _wl_save_backup()  # v41g: backup locale immediato
...
# TABS
# =========================================================================
# ── v41g: Menu sticky multi-riga — tutti i tab visibili su 3 righe ──────
st.markdown("""<style>
/* Sticky tab bar */
[data-testid="stTabs"] {
...
with tab_home:
    # ── v41g — ALERT BANNER: segnali STRONG visibili subito ──────────────
    try:
    ...
    # ── v41g — MERCATI LIVE GLOBALE con mini sparkline 5gg ─────────────────
    ...
    # ── v41g FEATURE 5 — MAPPA CALORE GLOBALE SVG ─────────────────────────
    with st.expander("🌍 Mappa Calore Globale — Performance indici mondiali v41g", expanded=True):
        try:
            ...
            if _live_for_map:
                ...
                # v41g: layout HTML flex puro - nessuna colonna Streamlit fissa
                _map_html = (
                ...
                for _rn, _ra in _map_regions.items():
                    _map_html += (
                        ...
                        + "".join(_map_card_v41g(l,s) for l,s in _ra) +
                        "</div></div>"
                    )
                _map_html += "</div>"
                _map_html += (
                    "<div style='margin-top:8px;display:flex;gap:5px;justify-content:center;flex-wrap:wrap'>"
                    + "".join(_map_card_v41g(l,s) for l,s in _map_macro) +
                    "</div>"
                )
    ...
    st.markdown('---')
    with st.expander('📰 NEWS & SENTIMENT v41g — Ultime news con score sentiment · TradingView Italia', expanded=False):
        _render_news_v41(df_ep)
    st.markdown('---')
    # ── v41g FEATURE 3 — HEATMAP SETTORIALE LIVE ──────────────────────────
    with st.expander("🌡️ Heatmap Settoriale Live — 11 settori × 3 periodi", expanded=True):
        @st.cache_data(ttl=300, show_spinner=False)
        def _sector_heatmap_home_v41g():
        ...
        _ht_period = st.radio("Periodo heatmap", ["1d","1m","3m"], index=0,
                               ...
                               label_visibility="collapsed")
        try:
            _ht_data = _sector_heatmap_home_v41g()
            if _ht_data:
    ...
    # ── v41g: Scanner Summary — segnali rapidi visibili dalla home ──────────
    if not df_ep.empty:
        st.markdown("---")
    ...
    # ── v41g: Suggerimenti ────────────────────────────────
    with st.expander('💡 Suggerimenti v41g — Novità e roadmap', expanded=False):
        st.markdown("""
**✅ Implementato in v41g:**
...
- 📱 Layout mobile-first con CSS container queries
    ...
    # ── v41g FEATURE 4 — Portfolio P&L dalla Watchlist ──────────────────────
    try:
    ...
    st.markdown('<div class="section-pill">🗓️ MACRO CALENDAR v41g — Fed · CPI · NFP · PCE</div>', unsafe_allow_html=True)
...
with tab_w:
    ...
    with st.expander("📈 P&L Tracker & Alert Engine v41g", expanded=False):
        ...
        with _pnl_col2:
            ...
            # ── v41g auto-notifica Telegram su FIRED ───────────────
                ...
                for _fa in [a for a in _alerts.values() if a.get('fired') and not a.get('notified')]:
                    try:
                        ...
                        _rqf.post(f'https://api.telegram.org/bot{_tgtv}/sendMessage',
                            json={'chat_id':_cgpv,'text':f"🚨 ALERT v41g\n{_fa['tkr']} {_fa['type']} {_fa['val']}"},timeout=5)
                        _fa['notified']=True
                    except Exception: pass

        # ── v41g pannello notifiche ─────────────────────────────
        with st.expander('📡 Canali Notifica v41g', expanded=False):
                ...
                if st.button('📨 Report P&L Telegram',key='pnl_tg_send'):
                    try:
                        ...
                        _rqt.post(f'https://api.telegram.org/bot{_tgtok}/sendMessage',
                            json={'chat_id':_tgcht,'text':'💼 Dashboard v41g P&L','parse_mode':'Markdown'},timeout=8)
                        st.success('✅ Inviato!')
                ...
                if st.button('🔔 Test Telegram',key='pnl_tg_test'):
                    try:
                        ...
                        _rqt2.post(f'https://api.telegram.org/bot{_tgtok}/sendMessage',
                            json={'chat_id':_tgcht,'text':'✅ Test OK v41g'},timeout=5)
            ...
            with _nc2:
                ...
                if st.button('📧 Report P&L Email',key='pnl_email_send'):
                    try:
                        ...
                        _m=_MT('Dashboard v41g P&L'); _m['Subject']='P&L v41g'; _m['From']=_emfrm; _m['To']=_emto
...
with tab_regime:
  ...
  <text x="155" y="113" font-size="8" fill="#00ff88">GREED</text>
    ...
    if not _sr_df.empty:
        ...
        # v41g: heatmap gia’ presente nella Home — bar chart disabilitato
        # st.plotly_chart(_fig_sr, use_container_width=True, key="sector_heatmap_v41")  # disabled v41g
...
# =========================================================================
with tab_mom:
    st.markdown('<div class="section-pill">⚡ MOMENTUM ALERTS v41g — Ticker · Nome · Tipo · Valore · RSI · Vol× · CSS · Priorità</div>',
                unsafe_allow_html=True)
