...
#btt-btn:hover{background:#0f766e!important;transform:translateX(-50%) translateY(-2px)!important;box-shadow:0 6px 24px rgba(13,148,136,.7)!important;}
#btt-btn:active{transform:translateX(-50%) translateY(0)!important;}
@media(max-width:768px){#btt-btn{left:auto!important;right:14px!important;transform:none!important;bottom:14px!important;}}
</style>
<button id="btt-btn" onclick="__btt()" title="Torna in cima">⬆ Torna su</button>
<script>
function __btt(){
  var p=window.parent;
  var d=p.document;
  // Target specifici Streamlit moderni
  var targets=[
    d.querySelector('[data-testid="stMainBlockContainer"]'),
    d.querySelector('[data-testid="stAppViewContainer"]'),
    d.querySelector('[data-testid="stMain"]'),
    d.querySelector('.stMainBlockContainer'),
  ...
  // Simula tasto Home su window.parent (funziona in molti browser)
  try{
    var ev=new KeyboardEvent('keydown',{bubbles:true,cancelable:true,key:'Home',keyCode:36,which:36});
    d.body.dispatchEvent(ev);
  }catch(e){}
  // scroll window parent
  try{p.scrollTo({top:0,left:0,behavior:'smooth'});}catch(e){}
  // Fallback: forza scroll su tutti elementi con scrollTop > 0 nel parent
  try{
    var all=d.getElementsByTagName('*');
    for(var i=0;i<all.length;i++){
      if(all[i].scrollTop>5){
        all[i].scrollTop=0;
...
# PAGE CONFIG
# =========================================================================
st.set_page_config(page_title="Trading Scanner PRO 44.0e",layout="wide",page_icon="🧠")
st.markdown(DARK_CSS,unsafe_allow_html=True)
st.markdown(BACK_TO_TOP_CSS,unsafe_allow_html=True)
st.markdown("# 🧠 Trading Scanner PRO 44.0e")
