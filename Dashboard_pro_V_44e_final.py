...
# ── V44e export helpers ────────────────────────────────────────────────────
def export_tv_txt(df, ticker_col='Ticker'):
    if df is None or df.empty or ticker_col not in df.columns:
        return b''
    out = []
    for tk in df[ticker_col].dropna().astype(str).unique():
        tk = tk.strip().upper()
        if not tk:
            continue
        if ':' in tk:
...
def make_csv_bytes(df):
    return df.to_csv(index=False).encode('utf-8') if df is not None and not df.empty else b''

def make_xlsx_bytes(df, sheet_name='Sheet1'):
    import io as _io
    buf = _io.BytesIO()
    with pd.ExcelWriter(buf, engine='openpyxl') as w:
        (df if df is not None else pd.DataFrame()).to_excel(w, index=False, sheet_name=sheet_name[:31])
    return buf.getvalue()
...
st.markdown("""
<style>
@media(max-width:1400px){[data-testid="stTabs"]>div:first-child button{font-size:.70rem!important;padding:4px 8px!important;max-width:130px!important;}}
@media(max-width:1100px){[data-testid="stTabs"]>div:first-child button{font-size:.65rem!important;padding:4px 7px!important;max-width:110px!important;}}
@media(max-width:850px){[data-testid="stTabs"]>div:first-child{gap:3px!important;overflow-x:auto!important;}[data-testid="stTabs"]>div:first-child button{font-size:.60rem!important;padding:3px 5px!important;max-width:90px!important;}[data-testid="stMetricValue"]{font-size:.95rem!important;}[data-testid="stMetricLabel"]{font-size:.62rem!important;}}
@media(max-width:580px){[data-testid="stTabs"]>div:first-child button{font-size:.53rem!important;padding:2px 4px!important;max-width:76px!important;}.block-container{padding-left:.6rem!important;padding-right:.6rem!important;}.stDataFrame{overflow-x:auto!important;}}
#bttv44e{position:fixed;bottom:28px;left:50%;transform:translateX(-50%);z-index:99999;background:#0f766e;color:#fff;border:none;border-radius:24px;padding:9px 22px;font-size:.82rem;font-family:Inter,system-ui,sans-serif;font-weight:600;cursor:pointer;box-shadow:0 4px 18px rgba(0,0,0,.35);opacity:0;transition:opacity .3s;pointer-events:none;}
#bttv44e.vis{opacity:1;pointer-events:auto;}
@media(max-width:600px){#bttv44e{bottom:14px;padding:7px 16px;font-size:.75rem;}}
</style>
<button id="bttv44e" onclick="bttTop()">&#x2B06; Torna su</button>
<script>
(function(){
  function g(){var a=['[data-testid="stAppViewContainer"] > section','.main > .block-container','.main','html'];for(var i=0;i<a.length;i++){var e=document.querySelector(a[i]);if(e&&e.scrollHeight>e.clientHeight+20)return e;}return document.documentElement;}
  window.bttTop=function(){g().scrollTo({top:0,behavior:'smooth'});window.scrollTo({top:0,behavior:'smooth'});};
  function c(){var e=g(),b=document.getElementById('bttv44e');if(!b)return;b.classList[(e.scrollTop||window.pageYOffset||0)>300?'add':'remove']('vis');}
  function a(){g().addEventListener('scroll',c,{passive:true});window.addEventListener('scroll',c,{passive:true});}
  [300,700,1500,3000].forEach(function(t){setTimeout(a,t);});
  new MutationObserver(function(){a();}).observe(document.body,{childList:true,subtree:true});
})();
</script>
...
""", unsafe_allow_html=True)
    ...
    # ── v43c: Suggerimenti ────────────────────────────────
    with st.expander('💡 Suggerimenti v44.0ec — Novità e roadmap — Novità e roadmap', expanded=False):
        st.markdown("""
...
with tab_news:
    ...
    # ── v44e BUBBLE METER & INDICATORI NON CONVENZIONALI ─────────────
