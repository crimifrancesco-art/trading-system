...
<script>
function __btt(){
  // Strategy: find ALL scrollable elements in parent document and reset them
  var docs=[window.parent.document, window.document];
  docs.forEach(function(doc){
    if(!doc) return;
    // 1. Named Streamlit containers
    var named=[
    ...
    named.forEach(function(el){
      if(el){ el.scrollTop=0; try{el.scrollTo({top:0,behavior:'smooth'});}catch(e){} }
    });
    // 2. Walk ALL elements with overflow scroll/auto and scrollTop > 0
    try{
      var all=doc.querySelectorAll('*');
      for(var i=0;i<all.length;i++){
        var el=all[i];
        try{
          var st=window.getComputedStyle ? window.parent.getComputedStyle(el) : null;
          if(!st) continue;
          var ov=st.overflow+' '+st.overflowY;
          if(/(scroll|auto)/.test(ov) && el.scrollTop>10){
            el.scrollTop=0;
...
# PAGE CONFIG
# =========================================================================
st.set_page_config(page_title="Trading Scanner PRO 44.0d",layout="wide",page_icon="🧠")
st.markdown(DARK_CSS,unsafe_allow_html=True)
st.markdown(BACK_TO_TOP_CSS,unsafe_allow_html=True)
st.markdown("# 🧠 Trading Scanner PRO 44.0d")
