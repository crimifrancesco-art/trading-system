...
if not only_watchlist:
    if st.button("🚀 AVVIA SCANNER PRO 45.05",type="primary",use_container_width=True):
...
(tab_home, tab_e, tab_p, tab_r, tab_conf,
 tab_ser, tab_fvpro, tab_macro, tab_crisis,
...
with tab_home:
    ...
    with st.expander("💡 Suggerimenti v45.05 — Stato e roadmap", expanded=False):
        st.markdown("""
...
- Usare COT e macro come filtri di contesto, insieme a prezzo, liquidità, volatilità, stop e position sizing.
    ...
    st.markdown("---")
    st.caption("Trading Scanner PRO · Versione V45.05 · Home aggiornata con stato release e roadmap")
...
with tab_macro:
    ...
    if _HAS_MACRO_REGIME and _fred_key_available:
        ...
        with st.expander("📝 Inserisci dati macro manualmente", expanded=True):
            with st.form("macro_manual_fallback_v45_05"):
                _m_fed = st.number_input("Fed Funds Rate (%)", value=5.00, step=0.05)
                _m_2y = st.number_input("US 2Y (%)", value=4.50, step=0.05)
                _m_10y = st.number_input("US 10Y (%)", value=4.00, step=0.05)
                _m_cpi = st.number_input("CPI YoY (%)", value=3.00, step=0.10)
                _m_core = st.number_input("Core CPI YoY (%)", value=3.00, step=0.10)
                _m_unemp = st.number_input("Disoccupazione (%)", value=4.00, step=0.10)
                _m_vix = st.number_input("VIX", value=20.00, step=0.10)
            ...
            if _m_submit:
                _m_curve = _m_10y - _m_2y
                if _m_vix >= 35:
                    _m_regime = "Crisis"
                elif _m_vix >= 25 or _m_curve < -0.50:
                    _m_regime = "Risk-Off"
                elif _m_vix >= 18 or _m_curve < 0:
                    _m_regime = "Caution"
                else:
                    _m_regime = "Risk-On"

                st.session_state["macro_manual_v45_05"] = {
                    "fed_funds": _m_fed,
                    "us_2y": _m_2y,
                    "us_10y": _m_10y,
                    "curve_10y_2y": _m_curve,
                    "cpi": _m_cpi,
                    "core_cpi": _m_core,
                    "unemployment": _m_unemp,
                    "vix": _m_vix,
                    "regime": _m_regime,
        ...
        _m_manual = st.session_state.get("macro_manual_v45_05")
        if _m_manual:
            st.success(
                f"🌡️ Regime manuale: {_m_manual['regime']} "
                f"· Fonte: {_m_manual['source']}"
            )
            st.json(_m_manual)
