...
# ═══ P1: STRONG banner — aggiunge Nome accanto al ticker ═══════════════
OLD_STRONG = (
...
# ═══ P2: Correlazioni — rimuovi seconda occorrenza (quella con pass) ════
# La P10 di v41d ha lasciato il blocco originale dopo il pass
OLD_CORR2 = (
    '    # v41d: correlazioni disponibili nel tab Settori\n'
    '    with st.expander("🔗 Correlazioni Asset — 30 giorni", expanded=False):\n'
    '        st.info("ℹ️ v41d: Correlazioni disponibili nel tab 🏭 Settori → dopo Ranking Settori.")\n'
    '        pass  # v41d: contenuto originale disabilitato in Home\n'
)
NEW_CORR2 = (
    '    # v41e: correlazioni rimosse dalla Home — disponibili nel tab Settori\n'
)
n2 = src.count(OLD_CORR2); src = src.replace(OLD_CORR2, NEW_CORR2, 1)
print(f"  P2  Correlazioni doppia:   {'OK' if n2 else 'SKIP'}")

# ═══ P3: NEWS — aggiunge Nome + link TradingView Italia ════════════════
OLD_NEWS = (
...
NEW_NEWS = (
    '        _tv_sym = str(n["Ticker"]).replace(".MI", "%3AMI")\n'
    '        _nome_n = str(n.get("Nome", "")).strip()[:20]\n'
    '        _nome_n_html = f" <span style=\'color:#6b7280;font-size:0.70rem;font-style:italic\'>{_nome_n}</span>" if _nome_n else ""\n'
    '        _c1.markdown(\n'
    '            f"<a href=\'https://it.tradingview.com/chart/?symbol={_tv_sym}\' target=\'_blank\' style=\'text-decoration:none\'>"\n'
...
print(f"  P3  NEWS Nome+link:        {'OK' if n3 else 'SKIP'}")

# ═══ P4: Top EARLY — aggiunge Nome ════════════════════════════════════
OLD_EARLY = (
    '                    st.markdown(\n'
...
# ═══ P6: Crisis RIEPILOGO — sanifica NaN prima di AgGrid ══════════════
OLD_AGGRID_R = '        go_r = gb_r.build()\n        try:\n            AgGrid(df_riepilogo,'
NEW_AGGRID_R = (
    '        # v41e: sanifica NaN/inf prima di AgGrid → evita JSON parse error\n'
    '        import math as _math_r, numpy as _np_r\n'
    '        _safe_r = []\n'
...
print(f"  P6  Crisis JSON NaN fix:   {'OK' if n6 else 'SKIP'}")
...
if failed:
    print(f"\n⛔ {len(failed)} patch fallite: {failed}"); sys.exit(1)
