...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="Trading Scanner – Versione PRO 5.0",
    layout="wide",
    page_icon="📊"
)

st.title("📊 Trading Scanner – Versione PRO 5.0")
st.caption("EARLY • PRO • REA‑QUANT • Rea Quant • Serafini • Regime & Momentum • Multi‑Timeframe • Risk & Portfolio • Export TradingView")
...
if st.button("🚀 AVVIA SCANNER PRO 5.0", type="primary", use_container_width=True):
...
# TABS
# =============================================================================
tab_e, tab_p, tab_r, tab_rea_q, tab_serafini, tab_regime, tab_mtf, tab_risk = st.tabs(
    [
        "🟢 EARLY",
        "🟣 PRO",
        "🟠 REA‑QUANT",
...
with tab_mtf:
    ...
    if df_ep.empty:
        ...
        if df_mtf.empty:
            ...
            # Export MTF generale (già presente)
            ...
            # Nuovi export: ALIGN_LONG e ALIGN_SHORT
...
with tab_risk:
    ...
    if port_df.empty:
            ...
            for tkr in tickers:
                try:
                    ...
                    if len(ret) > 1:
                        ...
                        vol_20d = float(vol_20d) * 100  # in percentuale annua approx
                    else:
                        vol_20d = np.nan
        ...
        if tot_val <= 0:
            ...
            # Export CSV portafoglio con rischio
...
st.download_button(
    ...
    data=xlsx_all_tabs,
    file_name=f"scanner_full_pro5_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
...
if lines:
    ...
    st.download_button(
        ...
        data=tv_text,
        file_name=f"tradingview_all_tabs_pro5_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
        mime="text/csv",
        use_container_width=True,
    )
else:
