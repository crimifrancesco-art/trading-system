...
# PAGE CONFIG
# =========================================================================
st.set_page_config(page_title="Trading Scanner PRO 41.0b",layout="wide",page_icon="🧠")
st.markdown(DARK_CSS,unsafe_allow_html=True)
st.markdown("# 🧠 Trading Scanner PRO 41.0b")
...
# ── v41b: Backup/Restore watchlist su file JSON locale ───────────────────────
...
def render_scan_tab(df,status_filter,sort_cols,ascending,title):
    ...
    if st.button(f"➕ Aggiungi a '{st.session_state.current_list_name}'",key=f"btn_{title}"):
        if not selected_df.empty and "Ticker" in selected_df.columns:
            ...
            _wl_save_backup()  # v41b: backup locale immediato
...
# TABS
# =========================================================================
# ── v41b: Menu sticky multi-riga — tutti i tab visibili su 3 righe ──────
st.markdown("""<style>
/* Sticky tab bar */
[data-testid="stTabs"] {
...
with tab_home:
    ...
    st.markdown('---')
    with st.expander('📰 NEWS & SENTIMENT v41b — Ultime news con score sentiment · TradingView Italia', expanded=False):
        _render_news_v41(df_ep)
    st.markdown('---')
    # ── v41b: Scanner Summary — segnali rapidi visibili dalla home ──────────
    if not df_ep.empty:
        st.markdown("---")
    ...
    st.markdown('<div class="section-pill">🗓️ MACRO CALENDAR v41b — Fed · CPI · NFP · PCE</div>', unsafe_allow_html=True)
...
# =========================================================================
with tab_mom:
    st.markdown('<div class="section-pill">⚡ MOMENTUM ALERTS v41b — Alert Pattern Tecnici in Tempo Reale</div>',
                unsafe_allow_html=True)
