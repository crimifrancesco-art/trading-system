...
def calc_quality_components(price, ema20, ema50, vol_ratio, obv_trend, atr_expansion, rsi_val):
    """
    Restituisce un dict con i singoli contributi al Quality Score
    per il radar chart. Valori normalizzati 0-1 per asse.
    """
    return {
...
def scan_ticker(ticker, e_h, p_rmin, p_rmax, r_poc, vol_ratio_hot=1.5):
    ...
    Novità v22.0:
    - chart_data: ultimi 60 giorni OHLCV + EMA20 + EMA50 + BB per candlestick inline
    - quality_components: dict per radar chart
    """
    try:
        ...
        quality_components = calc_quality_components(price, ema20, ema50, vol_ratio, obv_trend, atr_expansion, rsi_val)

        # Multi-timeframe
        try:
            data_w   = yf.Ticker(ticker).history(period="6mo", interval="1wk")
            c_w      = data_w["Close"]
            ema20_w  = float(c_w.ewm(span=20).mean().iloc[-1]) if len(data_w) >= 5 else None
            weekly_bullish = float(c_w.iloc[-1]) > ema20_w if ema20_w else None
        except Exception:
            weekly_bullish = None

        # ------------------------------------------------------------------
        # chart_data: ultimi 60gg per candlestick inline (NOVITÀ v22)
        # ------------------------------------------------------------------
        ...
        common = {
            ...
            "Prezzo": round(price, 2), "MarketCap": market_cap,
            "Vol_Today": int(vol_today), "Vol_7d_Avg": int(vol_7d_avg),
            "Currency": currency,
            ...
            "Weekly_Bull": weekly_bullish, "EMA50": round(ema50, 2),
            "Quality_Score": quality_score,
            "_quality_components": quality_components,   # per radar
            "_chart_data": chart_data,                   # per candlestick
        }
        ...
        if stato_early == "-" and stato_pro == "-":
            ...
            res_ep = {
                **common,
                ...
                "Stato": stato_ep, "Stato_Early": stato_early, "Stato_Pro": stato_pro,
            }
